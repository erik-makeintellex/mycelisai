...
// NewServiceWithDB creates a memory service from an existing *sql.DB.
// Primarily used by tests and subsystems that already hold a shared DB handle.
func NewServiceWithDB(db *sql.DB) *Service {
	return &Service{
		db:     db,
		events: make(chan *LogEntry, 1000),
	}
}
