	...
	r.tools["create_team"] = &InternalTool{
		...
		InputSchema: map[string]any{
			...
			"properties": map[string]any{
				...
				"initial_member_count":     map[string]any{"type": "integer", "description": "Requested initial member count. Current runtime creates lead-only teams, so this should be 1."},
				"recommended_member_limit": map[string]any{"type": "integer", "description": "Suggested cap after justified temporary additions."},
				"expansion_policy":         map[string]any{"type": "string", "description": "How operators or team leads may request additional temporary specialists."},
				"temporary_addition_guidance": map[string]any{
					"type":        "string",
					"description": "What the team lead must state before adding a temporary specialist.",
				},
