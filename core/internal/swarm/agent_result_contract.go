	...
	for _, issue := range issues {
		...
		if strings.HasPrefix(issue, "missing successful write for ") {
			...
			if strings.HasSuffix(strings.ToLower(target), ".html") {
				return "Write the entrypoint with write_file at " + target + " and include package_kind=project_package, package_folder=" + strings.TrimSuffix(target, "/"+pathBase(target)) + ", package_entrypoint=" + target + ", package_files=[index.html, README.md, PROOF.md, project-package.json]."
			}
