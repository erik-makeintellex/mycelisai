...
func (r *InternalToolRegistry) BuildContext(agentID, teamID, role string, teamInputs, teamDeliveries []string, currentInput string) string {
	...
	sb.WriteString("4. Check if specialist knowledge is needed -> `consult_council`\n")
	sb.WriteString("5. Check if actionable work should be delegated -> `delegate_task`\n")
	sb.WriteString("6. For software/dev tasks, prefer quick ephemeral code execution and bounded validation (`local_command`) before introducing new MCP dependencies\n")
	sb.WriteString("7. For web access tasks, default to coder-owned ephemeral web code first; use adaptive engine/query strategy\n")
	sb.WriteString("8. Check if installed MCP tools can fulfill remaining external integration requirements or provide easier execution\n")
	sb.WriteString("9. MCP Translation Procedure: map user intent -> operation/target/constraints/output, pick the narrowest installed MCP tool, then execute with minimal valid arguments\n")
	sb.WriteString("10. Check if data would benefit from visualization -> `store_artifact` with type=chart\n")
	sb.WriteString("11. For explicit image requests, call `generate_image`; if user asks to keep the image, call `save_cached_image`\n\n")
