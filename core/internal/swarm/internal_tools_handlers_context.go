...
func (r *InternalToolRegistry) BuildContext(agentID, teamID, role string, teamInputs, teamDeliveries []string, currentInput string) string {
	...
	sb.WriteString("- **Reflection / synthesis memory**: use governed synthesis entries for distilled lessons, inferred patterns, contradictions, shifts in user trajectory, and meta-observations about what is changing over time; keep them private/restricted unless explicitly widened.\n")
	...
	sb.WriteString("6. If the interaction reveals a durable lesson, inferred pattern, contradiction, user-trajectory shift, or meta-observation -> `load_deployment_context` with `knowledge_class=reflection_synthesis` and an appropriate `source_kind` such as `lesson`, `inferred_pattern`, `contradiction`, `trajectory_shift`, or `meta_observation`\n")
	...
	sb.WriteString("4. Store durable lessons, inferred patterns, contradictions, trajectory shifts, and meta-observations -> `load_deployment_context` with `knowledge_class=reflection_synthesis`\n")
