...
func manifestFromMCPLibraryEntry(category string, entry mcp.LibraryEntry) Manifest {
	return Manifest{
		...
		Metadata: map[string]any{
			...
			"configuration_kind":   entry.ConfigurationKind,
			"configuration_hint":   entry.ConfigurationHint,
			"connection_resource":  entry.ConnectionResource,
			"multiple_connections": entry.MultipleConnections,
