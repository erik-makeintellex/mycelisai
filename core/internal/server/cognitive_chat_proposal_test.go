...
func TestHandleChat_UsesFocusedTeamForSelectedTeamProposal(t *testing.T) {
	wireNATS := withNATS(t)
	s := newTestServer(wireNATS)
	s.Cognitive = &cognitive.Router{
		Config: &cognitive.BrainConfig{
	...
	if len(payload.Proposal.TeamExpressions) == 0 {
		t.Fatal("expected team expressions")
	}
	for _, expression := range payload.Proposal.TeamExpressions {
		if expression.TeamID != "marketing-team" {
			t.Fatalf("team expression team_id = %q, want marketing-team", expression.TeamID)
		}
	}
	wantSubjects := []string{
		"swarm.team.marketing-team.internal.command",
		"swarm.team.marketing-team.signal.status",
		"swarm.team.marketing-team.signal.result",
	}
	for i, subject := range wantSubjects {
		if len(payload.Proposal.NATSSubjects) <= i || payload.Proposal.NATSSubjects[i] != subject {
			t.Fatalf("nats_subjects = %#v, want prefix %#v", payload.Proposal.NATSSubjects, wantSubjects)
		}
	}
	for _, subject := range payload.Proposal.NATSSubjects {
		if strings.Contains(subject, "admin-core") {
			t.Fatalf("proposal nats_subjects leaked admin-core fallback: %#v", payload.Proposal.NATSSubjects)
		}
	}
}
