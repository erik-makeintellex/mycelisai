...
func TestHandleConfirmAction_CompletesVerifiedExecutionWithPlannedToolCalls(t *testing.T) {
	...
	if contractID != "33333333-3333-3333-3333-333333333333" {
		t.Fatalf("contract_id = %v, want persisted contract", data["contract_id"])
	}
	proofArtifactID, _ := data["proof_artifact_id"].(string)
	if proofArtifactID != "44444444-4444-4444-4444-444444444444" {
		t.Fatalf("proof_artifact_id = %v, want persisted proof artifact", data["proof_artifact_id"])
	}
	...
	if summary["contract_id"] != contractID || proof["contract_id"] != contractID {
		t.Fatalf("execution_summary contract linkage = summary:%v proof:%v want %s", summary["contract_id"], proof["contract_id"], contractID)
	}
	if summary["proof_id"] != proofArtifactID || proof["proof_id"] != proofArtifactID {
		t.Fatalf("execution_summary proof linkage = summary:%v proof:%v want %s", summary["proof_id"], proof["proof_id"], proofArtifactID)
	}
