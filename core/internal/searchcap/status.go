...
func (s *Service) Status() Status {
	...
	case ProviderSearXNG:
		...
		status.Sources = []Source{searchSource("searxng", "Self-hosted public web", "public_web", "self-hosted SearXNG endpoint", "none", "live", "public", "bounded_external", availability(status.Configured))}
		if !status.Configured {
	...
	case ProviderLocalAPI:
		...
		status.Sources = []Source{searchSource("local_api", "Operator-owned search API", "client_or_public_api", "configured local API endpoint", "service_managed", "live", "configured", "bounded_external", availability(status.Configured))}
		if !status.Configured {
...
func searchSource(id, name, sourceType, boundary, auth, mode, sensitivity, trust, status string) Source {
	return Source{
		ID:               id,
		Name:             name,
