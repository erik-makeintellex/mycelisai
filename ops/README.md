...
- On that WSL Docker path, `compose.up` and `compose.health` can auto-start a small WSL-host relay for `MYCELIS_COMPOSE_OLLAMA_HOST` so the Core container can still reach a Windows-hosted Ollama service through `host.docker.internal` when bridge containers cannot dial the Windows LAN IP directly.
