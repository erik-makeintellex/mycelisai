...
- Treat merge-readiness browser proof as a stability-sensitive path. `uv run inv ci.baseline` now calls Playwright with `--workers=2` against the built `next start` server, and `uv run inv ci.service-check --live-backend` keeps `--workers=1`, so the managed gate prefers repeatable results over peak local parallelism without making the default baseline unreasonably slow.
