...
- Compose `migrate` never repairs a retained volume by replaying removed development migrations. Back up retained data and use an explicitly supported upgrade path, or use `uv run inv compose.down --volumes` only for an intentional disposable rebuild. A fresh install reruns the complete schema and storage contract before reporting success.
...
- `lifecycle.first-boot-proof` intentionally resets the application database and clears generated workspace output roots, then proves Core/Interface can start from clean product state, keep Groups/Runs/Outcomes/conversations/vectors empty before the first ask, keep NATS JetStream empty, and restart without duplicating bootstrap rows. Requested Interface readiness is a hard gate; missing dependencies fail with the canonical install recovery. It preserves the Dockerized data-plane volumes and local workspace mounts.
- `lifecycle.down` treats repo-local Interface worker residue as part of the teardown contract, not just bound ports. A rewritten Linux `next-server` listener is owned only when its resolved working directory is the exact repo Interface directory; an `ss` fallback covers hosts without usable `lsof`, while foreign listeners remain untouched.
