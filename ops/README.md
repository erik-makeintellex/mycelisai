...
- Compose `up` and `migrate` now behave like the main `db.migrate` contract: they bootstrap forward only when the compose `cortex` schema is compatible through the V8.2 capability/proof/team-work tables, and they point to `uv run inv compose.down --volumes` for a truly fresh replay.
...
- `uv run inv db.migrate` is intentionally a forward-bootstrap helper now, not a replay-everything hammer. Once the schema is compatible through the V8.2 capability/proof/team-work tables, it skips replay and points operators to `uv run inv db.reset` for a clean rebuild path.
