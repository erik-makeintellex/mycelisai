...
`uv run inv compose.migrate` checks the current durable schema before deciding that a retained volume is compatible. Missing late contracts are applied selectively; this includes migration `061_config_documents.up.sql` and its configuration revision, activation, and activation-history tables. Pair migration with `uv run inv compose.storage-health` before treating the data plane as ready.
