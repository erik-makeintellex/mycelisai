...
`db.migrate` is forward-bootstrap aware: compatible schemas are not replayed as a cleanup mechanism, and compatibility now includes capability manifests, execution/proof artifacts, team-work tables, collaboration-group workspace folders, OutcomeProject/TeamRegistry ownership, and the search-source registry. Use `db.reset` when you intentionally need a fresh rebuild. Use `db.clear-runtime-context` before fresh Soma/team GUI proof when stale conversations, team work, run/proof handshakes, or temp memory would influence the operator experience; it dry-runs by default and requires `--yes` to delete rows.
