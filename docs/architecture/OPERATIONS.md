...
- governed search is deployment-time runtime config: use `MYCELIS_SEARCH_PROVIDER`, `MYCELIS_SEARXNG_ENDPOINT`, `MYCELIS_SEARCH_LOCAL_API_ENDPOINT`, and `MYCELIS_SEARCH_MAX_RESULTS` in Compose or Helm to select disabled, local-source, self-hosted SearXNG, self-hosted local HTTP API, or optional Brave-backed search posture without editing chart source
...
- Merge-readiness browser gates should bias toward repeatability. The managed readiness tasks now run Playwright with reduced worker counts on the local host path: `uv run inv ci.baseline` uses `--workers=1` against the built production Interface server path, while `uv run inv ci.service-check --live-backend` stays at `--workers=1`, restores the local bridge/core stack before the live governed Soma proof when that bridge is absent, and reuses an already-initialized `cortex` schema instead of replaying non-idempotent migrations every run. On Windows, managed Playwright UI servers bind to `127.0.0.1` to avoid IPv4/IPv6 loopback mismatch during start-mode proof.
...
| `uv run inv quality.max-lines --limit 300` | Enforce the source-tree `<=300 LOC` policy with temporary no-regression legacy caps and stale-cap detection |
...
| `ops/wsl_runtime.py` | `wsl.status`, `wsl.refresh`, `wsl.validate`, `wsl.cycle` | Guarded Windows-dev -> WSL-proof handoff, refresh, and validation tasks; `wsl.refresh` runs WSL git fetch noninteractively, tries a repo-local Git Credential Manager helper repair for GitHub HTTPS remotes when available, preserves generated `workspace/tool-cache`, `workspace/logs`, and `workspace/docker-compose` roots during source cleanup, and fails before reset/clean with SSH/HTTPS auth guidance when auth is still blocked; `wsl.validate` bootstraps `.env.compose` from the tracked example when needed, creates the configured Compose output-block host path, runs Compose-safe release-preflight, and then owns the Compose health/storage proof, live browser workflows, and Windows GUI probe; `--lane=service` and `--lane=release` map that preflight step to `--lane=runtime` because local lifecycle service-check is not the WSL Compose service gate |
