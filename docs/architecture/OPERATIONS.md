...
`uv run inv compose.migrate` checks the current durable schema before deciding that a retained volume is compatible. Missing late contracts are applied selectively; this includes migration `061_config_documents.up.sql` for configuration revision, activation, and activation-history tables plus `062_qa_fixture_config_documents.up.sql` for exact retained-configuration fixture ownership. The `062` downgrade fails closed when live configuration claims exist. Pair migration with `uv run inv compose.storage-health` before treating the data plane as ready.
