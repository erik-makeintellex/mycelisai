...
The separate Go service under `services/framework-runs` is the production Runs API implementation, not the Python facade. It requires `FRAMEWORK_RUNS_DATABASE_URL` for its independently owned database and `FRAMEWORK_RUNS_CORE_TOKEN` for the scoped Core bearer credential; optional bounds are `FRAMEWORK_RUNS_LISTEN_ADDRESS`, `FRAMEWORK_RUNS_MAX_RUNS`, and `FRAMEWORK_RUNS_LEASE_SECONDS`. Slice B has no Compose/Helm package and no executor injection: run it only for source-level service proof, expect `production_ready=false`, and do not point Core at it. Slice B2 owns disabled-by-default private packaging, secret references, persistence, network policy, probes, and rollback proof before any dispatcher work begins.
