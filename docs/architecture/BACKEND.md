...
Framework posture was rechecked on 2026-09-01. LangGraph remains the first optional worker-local adapter candidate. AG2 1.0 and AG2 Classic would each require an explicit adapter and conformance proof; CrewAI remains another adapter candidate rather than embedded authority; Microsoft Agent Framework remains a separate future evaluation target rather than an implemented backend. None is installed or enabled by this slice, and none may replace Mycelis' Outcome, policy, approval, audit, projection, or validation boundaries.
