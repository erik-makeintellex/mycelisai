...
A package team is complete only when it returns a retained project package under its own group folder with an entrypoint that another user or agent can open. A loose `.py`/`.js` file, copied request text, planning brief, or missing parent folder is not a delivered application. Valid completion returns to the Soma conversation as `Work complete`, names the retained deliverable, and provides a direct open action; run and transport evidence remain available through Details/Inspect.
