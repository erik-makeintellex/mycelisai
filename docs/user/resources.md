...
- client-owned search sources support persisted source records for readable name, source type, endpoint/base URL, boundary, scope, auth scheme, secret reference, sensitivity, trust defaults, live-vs-indexed mode, status, and recovery guidance; operator-managed records can be edited or removed, and `source_id` selection routes local-source, local-API, and SearXNG-compatible searches through scope/status guardrails before use; bearer/API-token references are recorded as secret references, while the runtime blocks authenticated source use until a safe adapter is available for that source shape
