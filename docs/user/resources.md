...
- client-owned search sources support persisted source records for readable name, source type, endpoint/base URL, boundary, scope, auth scheme, secret reference, sensitivity, trust defaults, live-vs-indexed mode, status, and recovery guidance; bearer/API-token references are the first supported auth shape, while service-specific query/header placement and OAuth2/client-credential metadata remain follow-on extensions
