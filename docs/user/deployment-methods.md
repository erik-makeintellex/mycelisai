...
After bring-up, Admin tools -> System -> Deployments should show the trust snapshot for the active lane. Treat `unknown` values as missing deployment metadata to fix in the runtime environment rather than as proof of a healthy release posture.
