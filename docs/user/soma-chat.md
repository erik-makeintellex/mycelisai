...
For a retained application, **Ask Soma for changes** returns to the originating conversation with the output title, retained reference, proof, owning team/work/run identity, and content digest when available. Soma validates that durable ownership before planning an update, keeps the original output unchanged, creates a separate revision target, inherits the original output contract, and adds the requested revision check. A stale or mismatched reference fails closed instead of silently editing an unrelated package.
...
Soma replies may also show one small action-state card inside the thread. Consecutive machine updates for the same work collapse into the latest state, while the full event history remains available through proof and inspection. The visible progression is `Planning`, `Queued`, `Building`, `Validating`, then `Ready` or `Recovery`; paused and operator-response needs remain explicit modifiers. Technical provider, tool, raw lifecycle status, and routing detail stay under `What happened`. The composer is the continuation path, so status events do not add repeated continuation buttons. Queued or Building work is not completion proof, and a retained interactive candidate remains Validating until its approved checks pass. When work stops, Soma explains that no usable result was produced and suggests plain conversational choices such as trying again, using another available service, or changing the request.
