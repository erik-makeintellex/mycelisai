...
2. Keep native PostgreSQL and NATS available when the local source stack needs real persistence or bus proof; run `uv run inv native-infra.up`, `uv run inv native-infra.status`, and `uv run inv db.migrate`, then containerize only after source proof is acceptable. Migration compatibility includes the current team-work `execution_mode` and `work_intent` columns, durable execution-dispatch outbox, and operator SSE event ledger so a green bootstrap cannot hide an older lifecycle, commit-before-dispatch schema, or non-replayable event stream. Confirmed delegated-work proof must also cover outbox enqueue/lease/retry tests, a fast `202 running` response, an enabled Soma composer after `Work started`, and either a validated retained output or an honest terminal degradation from the real worker.
