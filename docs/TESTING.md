...
UV_CACHE_DIR=/tmp/mycelis-framework-runs-cache uv run --no-sync pytest tests/test_framework_runs_sidecar.py tests/test_framework_runs_langgraph.py tests/test_k8s_chart_contract.py -q
```
...
uv run inv compose.storage-health
```
The migration compatibility probe must include current late tables and constraints. ConfigDocument changes require focused schema-mapping tests plus live proof that migration `061` creates revision/activation/history storage and migration `062` permits exact ConfigDocument fixture ownership on the retained PostgreSQL volume. Downgrade must refuse to discard live fixture claims.
## CI Pipelines
