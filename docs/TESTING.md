...
Every visible responsive slice checks one primary route scroll owner; bounded nested panes only; no document overflow, clipped labels, overlaps, unreachable fields, or fixed-UI occlusion; `44px` primary touch targets; keyboard/focus return and no hover-only action; URL/refresh/Back and selected-state continuity; loading/empty/degraded/offline/retry/long-content states; bounded composer growth; and visible-browser console, hydration, normalized request failure, and page-error review. Use `desktop-mobile-compression.spec.ts` for shared shell, Dashboard, Resources, and bounded-layout regressions. Its Dashboard output-review proof opens the overlay at desktop and compact widths, checks that package controls wrap below content, verifies panel and document widths have no horizontal overflow, and retains screenshots for visual inspection. Dashboard Outcome proof in `missions.spec.ts` runs all four reference viewports and verifies the drawer stays closed by default, overlays instead of narrowing Soma, becomes a full-width compact sheet, traps focus, closes through its control/backdrop/Escape, returns focus, and leaves the composer reachable. Groups list-detail and Back proof belongs in `groups.spec.ts`; Memory compact tab, URL, refresh, Back, semantic-search degradation, overflow, and browser-error proof belongs in `memory-ui-tabs.spec.ts`. Run the affected route at `390x844`, then inspect medium, workspace, and wide screenshots before accepting shared component changes. PWA/native-shell proof begins only after that surface becomes an approved target.
...
| --- | --- | --- |
| Canonical MVP workflow | headed/live Soma request -> proposal -> approval -> run -> output -> proof/audit -> revisit | `ACTIVE` |
