...
- [Transition Hygiene Gate](#transition-hygiene-gate)
- [Framework Runs Neutral Gate](#framework-runs-neutral-gate)
...
Every architecture-transition slice runs `UV_CACHE_DIR=/tmp/mycelis-hygiene-cache uv run --no-sync pytest -q tests/test_tooling_hygiene.py tests/test_cleanup_tasks.py tests/test_quality_tasks.py tests/test_k8s_config_parity.py tests/test_k8s_chart_contract.py tests/test_canonical_workspace_docs.py tests/test_trusted_outcome_docs.py tests/test_docs_links.py tests/test_workflow_contracts.py`, `git diff --check`, and `UV_CACHE_DIR=/tmp/mycelis-hygiene-cache uv run inv quality.max-lines --limit 385`. Chart proof must assert that every packaged config file is mapped to one canonical source and mounted when runtime-owned; retired-path proof covers replaced aliases, generated stubs without a service owner, personal/session artifacts, and renamed tests/docs. Cleanup proof must preserve secrets, runtime logs, complete workspaces, Compose data, Docker volumes, PostgreSQL, NATS, and the active Python environment.
...
```bash
UV_CACHE_DIR=/tmp/mycelis-framework-runs-cache uv run --no-sync pytest tests/test_framework_runs_langgraph.py -q
```
Configuration/client proof must cover the `central` default; fail-closed unknown and retired session/tool/fallback fields, old names, raw secrets, credential URLs, unsupported modes, invalid paths, unresolved `env:` references, and no automatic external-to-central fallback; exact health/capabilities/create/get/events/approval/stop vocabulary; no `RunFinalizer`; missing optional config; and explicit invalid `MYCELIS_ENGINE_CONFIG_PATH` without fallback.
...
The focused mock-facade integration path is commit Mycelis authority -> post-commit create -> progress -> Core-owned approval -> completion candidate -> Mycelis retention/validation/proof, plus duplicate/restart reconciliation, stop, and failure. Python proof retains bounded-store, identity/idempotency/conflict, correlation/events, cancellation, central approval, candidate-only completion, dependency-gated worker-local LangGraph, and no generic approval-resume claims. Chart proof keeps source/image/Helm runtime configuration equal and secret-free. Agent Server is not started. Certification order is neutral gate first, existing LangGraph driver second, optional CrewAI compatibility/import work later, and AG2 or Microsoft Agent Framework only through a separate evaluation. The current LangGraph regression does not satisfy that certification gate. The canonical PRD's [P0.10 delivery referential](architecture-library/MYCELIS_CANONICAL_PRD.md#p010-framework-execution-delivery-referential) owns the exact slice dependencies and proof gates.
...
```
The Go gate must use a local mock upstream and prove the configured provider id survives the call; the upstream model, opaque response-body id, prompt tokens, completion tokens, and aggregate total are passed through exactly; missing accounting fields remain absent/zero rather than being estimated; tool calls keep their normalized Mycelis payload; authentication uses only the resolved provider secret; and gateway failures expose neither raw upstream bodies nor secrets. Router proof must show the added accounting survives configured-provider normalization while `tokens_used` remains the aggregate compatibility field. The configuration gate must prove source and Helm defaults declare `litellm` as disabled, use the existing `openai_compatible` type, reference `LITELLM_PROXY_API_KEY`, and contain no raw proxy or upstream credential. It must also prove enabling the chart requires an explicit endpoint and existing Secret while leaving proxy installation external. No test in this slice installs LiteLLM, calls a hosted model, starts services, or claims budget/rate-limit enforcement.
...
Unit proof must cover endpoint validation, public HTTPS enforcement, redirect rejection, bounded timeouts and response size, liveness/readiness, positive and negative scoped-key authentication, exact alias matching, normalized failures, and absence of secrets or response bodies in output. A real loopback HTTP test must exercise the exact health/models GET sequence, prove the scoped key appears only on authenticated model discovery, prove no completion request occurs, and show a real redirect is rejected without forwarding the key. Go proof must cover the explicit gateway marker and boundary, keyed pseudonymous correlation for authoritative swarm scope, omission for ordinary providers, exact upstream response/accounting pass-through on normal and tool-call responses, and fail-closed gateway errors with no legacy cross-provider reroute. A live preflight pass proves only reachability, readiness, authentication, and alias visibility. The opaque response-body id is not gateway request identity or spend-log correlation. Hosted inference, non-swarm correlation, cost reconciliation, fallback isolation inside the proxy, retry/rate-limit behavior, logging/callback/cache posture, and persistence/Redis recovery remain separate required evidence.
...
Every visible responsive slice checks one primary route scroll owner; bounded nested panes only; no document overflow, clipped labels, overlaps, unreachable fields, or fixed-UI occlusion; `44px` primary touch targets; keyboard/focus return and no hover-only action; URL/refresh/Back and selected-state continuity; loading/empty/degraded/offline/retry/long-content states; bounded composer growth; and visible-browser console, hydration, normalized request failure, and page-error review. Use `desktop-mobile-compression.spec.ts` for shared shell, Dashboard, Resources, and bounded-layout regressions. Use `soma-human-first-journey.spec.ts` for untouched first-viewport composer reachability, user-work navigation language, zero default runtime vocabulary, overlay width stability, and substantial-output promotion at desktop and compact widths. Use `soma-work-attention.spec.ts` to prove repeated same-work machine events collapse into one plain-language direction state, the composer remains the only continuation path, technical detail stays disclosed, and desktop/compact layouts do not overflow. Dashboard Outcome proof in `missions.spec.ts` runs all four reference viewports and verifies the drawer stays closed by default, overlays instead of narrowing Soma, becomes a full-width compact sheet, traps focus, closes through its control/backdrop/Escape, returns focus, and leaves the composer reachable. Groups list-detail and Back proof belongs in `groups.spec.ts`; Memory compact tab, URL, refresh, Back, semantic-search degradation, overflow, and browser-error proof belongs in `memory-ui-tabs.spec.ts`; mobile Docs list-to-article and local table/code scrolling belongs in `docs-mobile.spec.ts`; Settings proof must click every guided setup action and confirm the named tab, URL, focused visible content, aligned action row, and Resources web-access destination in `settings.spec.ts`, alongside its mobile single-column and nested-overflow checks. Run the affected route at `390x844`, then inspect medium, workspace, and wide screenshots before accepting shared component changes. PWA/native-shell proof begins only after that surface becomes an approved target.
...
1. Source and contract proof from the Windows repo first: `uv run inv core.test`, Interface gates, docs tests, `api.delivery-proof` when Core is live, and `uv run inv quality.max-lines --limit 385`; authored source files target 350 lines with a 10% tolerance, and capped legacy files in `ops/quality_legacy_caps.txt` must match current counts.
2. Keep Dockerized PostgreSQL and NATS available when the local source stack needs real persistence or bus proof; run `uv run inv compose.infra-up`, `uv run inv compose.infra-health`, and `uv run inv db.migrate`, then containerize Core/Interface only after source proof is acceptable. The single `001_current_schema.sql` installer applies atomically only to an empty public schema. A fully compatible retained schema is an idempotent no-op; any nonempty partial or incompatible schema must fail before installer execution and require a backup plus intentional reset or an explicitly supported upgrade path. Schema proof must cover old-chain/current-baseline parity, source and Compose empty install, compatible no-op with row preservation, partial-schema rejection, post-install compatibility, and clean first boot. Confirmed delegated-work proof must also cover outbox enqueue/lease/retry tests, a fast `202 running` response, durable `Team accepted work` after consumer receipt, restart replay rejection, stable projected result identity, an enabled Soma composer while work runs, and either a validated retained output or an honest terminal degradation from the real worker. Recovery tests must distinguish ordinary overdue work from accepted external mutations: an unknown external outcome requires verification and must reject generic start, pause, resume, and recover controls. The UI must suppress enabled retry and send the operator back to Soma with verification guidance; a new governed request may expose same-key retry only after verification confirms no commit and the capability explicitly supports safe idempotent reuse.
   Shared-broker proof must additionally inspect NATS client names for the configured `MYCELIS_NATS_SERVICE_ID`, register one concrete external input subject, reject a duplicate claim with `409`, publish one event, verify its unfiltered durable buffer, and confirm the raw external payload was not routed as operator intent.
...
| --- | --- | --- |
| Canonical MVP workflow | headed/live Soma request -> proposal -> approval -> run -> output -> proof/audit -> revisit | `COMPLETE` |
...
| TeamWorkItem/TeamInteraction/TeamStatusEvent | Go/API proof for non-active team creation, durable async work, retained-output completion, missing-output degradation, action transitions, Event Spine status events, and archived-row hiding | `IN_REVIEW` |
...
| Deployment trust | `System -> Deployments` proof for roots, commit, endpoint/runtime posture, proof lane, and recovery state | `IN_REVIEW` |
Related active contract: [Mycelis Canonical PRD](architecture-library/MYCELIS_CANONICAL_PRD.md). Current GUI proof status: live Soma governance/team/playable-output flows are green; `team-output-content-live.spec.ts` proves chat -> confirm-action -> durable team work -> status events -> reveal API readback -> focused Dashboard output digest -> Groups Workflow Log review -> Resources folder readback; unit proof covers proof-linked approval feedback, current-work lane summary, latest-output file/folder guidance, focused Dashboard review-inbox recovery cards, full Teams list-detail review inbox, and closed-panel dashboard compression; `desktop-mobile-compression.spec.ts` includes a short-viewport Dashboard composer reachability guard plus the Dashboard `Review work` focused-inbox panel that checks readability, queue summary, `Open inbox`, and recovery/clear primary actions; `teams.spec.ts` covers `/teams?view=work` list-detail review with compact rows, selected detail, and recovery/output actions.

## Full GUI Coverage Matrix
Current browser workflow requirements live in [Mycelis Canonical PRD](architecture-library/MYCELIS_CANONICAL_PRD.md). Keep this document as the validation policy entrypoint rather than a route-by-route duplicate.
Minimum route families under active proof: `/login`, `/dashboard`, organization re-entry, `/groups`, `/teams`, `/teams/create`, `/resources`, `/memory`, `/system`, `/settings`, `/runs`, run detail/chain, and legacy redirects.
Current local-source GUI certification posture:
- Green focused proof includes signed-in `/login` -> `/dashboard` entry, Soma governance, Resources Deliverables, Capabilities, Settings, Accessibility baseline, homepage/navigation/layout/package/team/groups/system/mobile/compression, and the new-user acceptance surfaces in [Mycelis Canonical PRD](architecture-library/MYCELIS_CANONICAL_PRD.md). Dense-panel proof uses `clickVisibleControl` in `interface/e2e/support/click-visible-control.ts`, Settings/Auth follows the Advanced Settings -> Auth Providers and Resources contract, and the accessibility baseline keeps critical checks active while `color-contrast` remains split out after axe timeouts in headed Chromium.
...
- `new-user-ui-sweep.spec.ts` is the integrated fresh-user navigation gate. It starts from an unauthenticated stale work URL, proves login lands at Soma, then uses visible navigation through Work's advanced group setup, Resources Deliverables/Capabilities/Exchange, Memory, Help, Settings, and back to Soma at desktop and compact-phone widths. Keep page exceptions, hydration/React console errors, `5xx` responses, and document-level horizontal overflow at zero. Run it visibly against the source stack with `uv run inv interface.e2e --headed --server-mode=external --project=chromium --workers=1 --spec=e2e/specs/new-user-ui-sweep.spec.ts`.
## Clean Run Discipline
- Stop prior Core/Interface services before runtime or integration tests with `uv run inv lifecycle.down`; Dockerized PostgreSQL and NATS remain reusable and are inspected with `uv run inv compose.infra-health`. Linux teardown recognizes Next's rewritten `next-server` listener only through an exact repo `interface/` working-directory match, including an `ss` listener lookup when `lsof` is unavailable; foreign listeners remain fail-closed and untouched. Use `lifecycle.down --include-data-plane` only when the proof requires the dependency containers stopped too; it preserves named volumes.
...
- Live acceptance that creates retained resources must run Core with `MYCELIS_QA_FIXTURE_MANAGEMENT=true`, create one `/api/v1/testing/fixture-scopes` record before the journey, and send the returned scope ID as `X-Mycelis-QA-Fixture-Scope` on browser requests. Core records exact organization, run, newly created Group/team/workspace, Outcome, and artifact provenance at trusted creation boundaries; helper discovery may re-submit those claims but cannot claim a merely recent same-tenant record. Before scoped team creation, Core rejects an already active team, records from another or unknown run, collaboration-group membership, or an existing workspace unless that team is already owned by the same scope. Current-run visibility records may exist before asynchronous team creation. Team and workspace ownership then comes only from an actual `create_team` result with `status=created` and is committed independently of the outer execution transaction so later rollback cannot erase cleanup provenance. Team-only artifact correlation remains rejected. Creation claims and purge share one PostgreSQL advisory fence, including async and partial-failure execution. Cleanup first requests `confirm:false` when operator review is needed, then uses `confirm:true`; confirmed cleanup serializes and resumes interrupted purges, stops producers, never guesses ownership from names or clears shared NATS state, and releases claims only after successful cleanup. Canonical live journeys keep creation inside `try/finally`, attempt purge even when discovery or evidence attachment fails, and clear the fixture header afterward.
- Project-package worker corrections request one missing contract gap at a time. Missing files are written before readback, entrypoint corrections may address all related structural issues together, and the worker must emit exactly one tool-call JSON object because only the first object is executed. Browser-package prompts require stable IDs or approved validation markers for event handlers; positional selectors such as `nth-child` are invalid proof dependencies.
...
uv run inv interface.e2e --server-mode=start --project=chromium --workers=1
...
uv run inv interface.e2e --headed --live-backend --server-mode=external --project=chromium --workers=1 --spec=e2e/specs/soma-outcome-template-retained-live.spec.ts
```
The retained Outcome Template journey must save and approve one exact revision, activate it, reload the Soma workspace, apply the active revision to work with matching record id/version/digest, and purge only the fixture-owned revision while restoring any prior activation. Adversarial tests must reject same-scope identity or digest substitution, missing or foreign request authority, and concurrent receipt reordering. A timeout, duplicate revision, stale activation, or remaining fixture claim fails the journey.
...
4. Run local-source deployment/active-work proof only after Core and Interface are already running from source against the Dockerized PostgreSQL/NATS data plane; do not start full Core/Interface Docker app stacks for this lane. `system-deployments.spec.ts` uses the live endpoint when `--live-backend`, `PLAYWRIGHT_LIVE_BACKEND=1`, or `PLAYWRIGHT_SYSTEM_DEPLOYMENTS_LIVE=1` is set. `active-work-api.spec.ts` writes one durable proof work item, posts one bounded `/work/ask`, verifies either accepted async queued dispatch, `output_ready`, or degraded timeout/offline/unreadable state, and reads status events/interactions back only when `PLAYWRIGHT_TEAM_WORK_API=1` or `PLAYWRIGHT_ACTIVE_WORK_API_LIVE=1` is set; it fails migrated-table regressions while skipping when Core/auth/PostgreSQL are unavailable. `active-work-ask-live.spec.ts` is the stricter browser gate: with `PLAYWRIGHT_TEAM_WORK_GUI_LIVE=1`, a responsive runtime team, and an interactive local model, `/teams` must submit Ask Team, avoid a blocking browser wait, refresh the durable row, and show either queued async continuity, visible reply proof, or degraded recovery truth in the Active Work Lane.
Use `npm test -- ActiveWorkLane TeamsPage teamWorkProjection OutputWorkbench useDurableTeamWork MissionControlChat.teamContinuation --maxWorkers=1` when Soma Active Work Lane, Ask Team/Respond controls, ready-team continuation prompts, durable team-work projection, or retained team-output workbench mapping changes. When output Reply or delivered-output re-entry changes, prove the compact `Continuing from` indicator, empty/typed composer, `/api/v1/chat` `continuation_context`, and response `continuation_intent` for ordinary file outputs and `project_package` outputs; also prove ordinary chat omits that context, the original output/proof remains visible, and any mutation still waits for normal approval. Use `cd core; go test ./pkg/protocol -run TeamWork -count=1` and `cd core; go test ./internal/server -run TeamWork -count=1` when durable team-work ask/action semantics, transition validation, status events, Event Spine bridge emission, or interaction persistence change. When async team completion changes, focused proof must cover retained-output success with a `team_signal_result` proof artifact and stamped output refs, plus deliverable result signals that claim completion without `outputs[]` or `output_refs[]`; the latter must stay recoverable as `degraded` with `missing_retained_output` rather than becoming trusted `output_ready`. Package proof must also reject missing physical entrypoints or local dependencies as `incomplete_deliverable_files`, reject required interaction without a visible/state-changing input path as `unverified_primary_interaction`, prove corrected same-ID output replacement before validation, prove synchronous confirmed packages outside the assigned team workspace or without readable entrypoints remain degraded without trusted refs, and stop/recreate the same team id to prove one command produces one result after every old team/agent NATS subscription is removed.
Use `uv run inv interface.e2e --headed --server-mode=external --project=chromium --workers=1 --spec=e2e/specs/soma-media-artifacts.spec.ts` when chat media rendering, saved-media downloads, local storage-folder reveal controls, or local media provider behavior changes. The focused browser proof should show image previews, saved paths, and an operator-visible control that opens the mounted storage location for generated content. For direct private Pinokio Forge proof, start Forge with API mode enabled, set `MYCELIS_MEDIA_TYPE=forge` and `MYCELIS_MEDIA_ENDPOINT=http://127.0.0.1:7860`, and verify `/sdapi/v1/options` before generating; no media gateway is required. Also prove that an open Forge UI with a `404` API route produces one setup instruction before approval. For ComfyUI, start the reviewed gateway/workflow path at `http://127.0.0.1:8001/v1`. Public upstream blocking, ComfyUI `/prompt` -> `/history/{prompt_id}` -> `/view` transmission/retrieval, and `response_format=url` rejection remain covered by `uv run pytest tests/test_media_gateway.py -q`.
Use `npm test -- WorkspaceExplorer MCPToolRegistry MCPLibraryBrowser MCPServerCard --maxWorkers=1` from `interface/` when Capabilities, filesystem MCP, Deliverables browsing/open-folder behavior, or MCP output generation contracts change. Use `npm test -- DeploymentContextPanel ResourcesPage --maxWorkers=1` when Resources -> Deployment Context intake compression changes; proof should verify the content/classification/scope tabs, bounded loaded-context list, and unchanged POST payloads.
...
Use `uv run inv interface.e2e --project=chromium --workers=1 --server-mode=external --spec=e2e/specs/soma-proposal-mode.spec.ts` when proposal approval, confirm-action failure handling, degradation metadata, or failed-run recovery UI changes. The focused browser proof must show the failed run remains reviewable, the compact result card says `Needs review`, the recovery details disclose what failed, what remains available, what is not reliable, and the safe next step, and success proof labels are absent.
Use `npm test -- ActiveWorkLane.review.test.tsx TeamsPage.review.test.tsx RunReceipt.test.tsx MCPToolRegistry.test.tsx ResourcesPage.test.tsx --maxWorkers=1` when unified recovery queue language changes across failed/degraded team work, run receipts, and capability repair states. Pair it with the focused browser proof for the visible route that changed: `teams.spec.ts` for the review inbox, `docs-and-runs.spec.ts` for run receipts, `mcp-connected-tools.spec.ts` for Resources/Capabilities, or `resources-exchange.spec.ts` for Exchange handoffs.
...
```
Use `team-execution-live.spec.ts` when team execution, retained file/code outputs, group visibility, run-conversation proof, or team NATS proposal subjects change. The native Core proof path now infers `MYCELIS_BACKEND_WORKSPACE_ROOT` from the loaded `.env`/process `MYCELIS_WORKSPACE`: absolute roots are used directly, while repo-local `./workspace` maps to `core/workspace`; Compose or split-checkout proof can still set `MYCELIS_BACKEND_WORKSPACE_ROOT` or `PLAYWRIGHT_BACKEND_WORKSPACE_ROOT` explicitly, and PVC-backed K8s proof should use `PLAYWRIGHT_BACKEND_WORKSPACE_PROBE=k8s`. The proof asks Soma for explicit expected output criteria, creates a runtime team, approves governed execution, writes a small browser-game HTML file through the backend workspace, confirms retained `team` and `code` outputs with the exact workspace-viewer href, opens the generated game from the retained output link, verifies the page title and initial score, clicks it in a browser page, checks the final score, checks `/api/v1/runs/{id}/conversation`, and verifies the team appears in `/groups`. Use `team-output-content-live.spec.ts` when the question is whether teams are producing meaningful content as specified: it creates a team through Soma, approves the work, opens the retained browser output and local folder control in headed Chromium, verifies deliverable content, play-tests a code-graphics game score, checks run-conversation tool proof, confirms the team is reviewable in Groups, verifies the selected group's Workflow Log contains the generated output path, and verifies Resources can open the output folder.
Group output cleanup proof must remove dead durable refs after owned workspace deletion while preserving artifacts and refs outside that exact group workspace when a team is shared.

The canonical group workflow-log API proof must show
`GET /api/v1/groups/{id}/workflow-log` returns one read-only timeline
with group brief/lifecycle state, durable team-work rows, retained output refs,
run/proof/audit refs, latest group message, and degradation/recovery entries.
The UI should no longer need to fan out across `/groups/{id}/outputs`,
`/groups/lifecycle`, `/groups/monitor`, and `/teams/{team_id}/work` to build the
default group review stream.
Update `interface/e2e/support/groups-workspace.ts`,
`interface/__tests__/teams/GroupManagementPanel.testSupport.ts`,
`GroupManagementPanel` unit coverage, and `groups.spec.ts` to prefer the
canonical endpoint while preserving a visible degraded/fallback state if the
endpoint is unavailable. Focused proof should include
`go test ./internal/server -run "TestHandleGroup(WorkflowLog|Outputs|Lifecycle)" -count=1`,
`npm test -- --run __tests__/teams/GroupManagementPanel.test.tsx __tests__/teams/GroupManagementPanel.tabs.test.tsx`,
TypeScript, docs links, and headless plus headed `groups.spec.ts`.
When complex generated projects such as playable games are packaged as `project_package` outputs, focused proof should verify the current output-first card and Groups retained-output pane show the package title, entrypoint, folder, file list, validation/proof text, `Open file`, workspace `Open folder`, `Open in Resources`, support files (`README.md`, `PROOF.md`, `project-package.json`), and collapsed `More outputs and verification` details. Routing proof must include a natural request for a web page drawn with SVG and verify that it produces an `app_package` plan without `generate_image`, `save_cached_image`, or image-provider readiness. The compact Dashboard digest must keep title/health readable above a separate wrapping action row at laptop width. Worker-contract proof must cover progressive required-file writes beyond four corrections, the hard 20-iteration ceiling, referenced local dependency closure, rendered marker attributes, inline and referenced-JavaScript interaction handlers, automatic exact entrypoint readback, no extra provider turn after runtime readback, stale readback invalidation after repair, divergent read rejection, full runtime-owned reads larger than the normal model-visible truncation limit, and bounded runtime-owned package recovery when the configured provider fails before or after partial file evidence. The planner proof must also verify that a natural Soma ask which creates a team and asks that team to build a meaningful deliverable produces one governed plan with `create_team`, retained `TEAM_EVOCATION.md`, retained `RESEARCH_COUNCIL_HANDOFF.md`, and delegated implementation work carrying a `project_package` result contract; simple team creation and exact inline file-generation requests must remain narrower. The focused natural-delivery benchmark is `soma-natural-delivery-routing-live.spec.ts`: it must ask Soma naturally, approve, steer the active work while Soma remains available, wait on durable work state, open the retained package through the Mycelis output canvas, exercise controls inside its iframe, prove a visible state change, and purge only the owned fixture scope. The broader P0 playable-game benchmark remains `soma-browser-game-p0-live.spec.ts`: it must verify code-generated graphics, movement, collision, hazards/enemies, health, key, locked door, win/fail states, and restart. Input proof must sample visible state while the requested key or pointer action is active, then release input in cleanup; sampling only after release can miss valid held-input behavior. Compose live proof defaults `MYCELIS_WORKSPACE_REVEAL_DRY_RUN=1`, so `Open folder` proves the retained mounted path without launching a host file explorer from the Core container. Unit/component proof can stay mocked with `npm test -- MissionControlChat.executionSummary GroupManagementPanel --maxWorkers=1`; backend proof should include confirmed `write_file` and `store_artifact` artifact-envelope paths with `go test ./internal/server ./internal/swarm ./internal/artifacts ./pkg/protocol -run "Test(BuildDirectChatExecutionSummary|ExtractToolOutputArtifacts|ArtifactType|ExecutionOutputsFromToolResults|ArtifactResultPayload|ExecutionOutputsFromArtifacts|ChatResponsePayload)" -count=1`; live proof should use the headed `soma-natural-delivery-routing-live.spec.ts`, `soma-browser-game-p0-live.spec.ts`, `team-output-content-live.spec.ts`, or `team-execution-live.spec.ts` path to open and exercise the retained browser output.

Runtime-owned package recovery must not turn structural acceptance into semantic acceptance. A generic scaffold may satisfy only a generic primary-interaction contract; when approved criteria name substantial behavior, domain content, or multiple user-visible states, provider failure must remain degraded instead of presenting the scaffold as the requested result. The P0 playable-game journey opens the result inside the Mycelis output-canvas iframe and verifies code-generated graphics, movement, collision, hazards/enemies, health, key, locked door, win/fail states, restart, and generated audio.

Mutation-tool parsing proof must include truncated JSON, invalid JSON, fenced valid calls, and missing required arguments. A malformed `write_file` must never reach executor lookup or persist bytes; the loop may ask the provider for one corrected complete call. Semantic completion proof must show that acceptance criteria survive proposal, WorkIntent, team handoff, and validation; a generic browser probe cannot complete semantic-required work without a passing evidence reference for every criterion.

Worker prompt proof must also assert that the rendered TeamAsk contains one authoritative package contract, suppresses only exact duplicate acceptance/proof items, and keeps genuinely additional requirements. Request-shape tests must keep system instructions before user content. Initial, empty-response, tool-feedback, tool-result, policy, and result-contract inference paths must use the shared deadline; a second malformed or missing-argument response for the same tool target must stop without execution. Sanitized telemetry may report request size, attempt, timing, provider routing, and output budget, but tests must prove it never logs message content or tool arguments. Focused coverage lives in `agent_trigger_test.go`, `agent_result_contract_test.go`, and `agent_inference_bounds_test.go`; live acceptance still requires one single-owner `soma-browser-game-p0-live.spec.ts` run with exact fixture cleanup.

Project-package history proof must assert a fixed base of system messages plus the authoritative user contract, at most one path-only evidence ledger, and one bounded latest event. It must exclude generated file contents and superseded malformed responses while retaining the latest operator interjection. Successful tool execution renews one unsafe-call correction; repeated malformed output without intervening success still stops. Game semantic preflight must reject the retained live anti-patterns: `canvas.textContent`, marker-only buttons, decorative animation loops, label-only gameplay actions, and movement state not consumed by canvas drawing. Generic DOM applications remain outside the game-only posture. A negative live run is accepted as failure evidence only when it ends `degraded/result_contract_unsatisfied`, never reaches browser validation or playable canvas claims, and purges its exact scope.

Game-planning proof must retain one accountable `default.builder` owner, name the context analyst, optional media creator, and reviewer as bounded targets, and avoid creating concurrent package writers. The result contract must require `index.html`, `game.js`, `styles.css`, `README.md`, `PROOF.md`, and `project-package.json`; correction metadata must preserve that exact list. Prompt proof must instruct one missing file per mutation and keep gameplay state/render logic in referenced JavaScript, while semantic proof must continue to inspect the combined entrypoint and local script.

Recovery-copy proof must inject hostile validator and worker diagnostics containing call logs, selectors, DOM fragments, URLs, absolute paths, ANSI escapes, provider/model names, and tool vocabulary. Primary status headline/detail, interaction summary, and recovery options must contain only the canonical failure-class copy and one same-team repair action; raw validation evidence remains reachable only through deliberate Inspect/proof. Soma UI proof must show **Output is not playable yet**, state that the retained candidate is unverified and not ready to use, expose one **Ask Soma to have the same team repair it** action, and omit Ready, Work complete, and Open app. Headed `soma-work-attention.spec.ts` verifies the raw detail stays collapsed, the generic recovery path remains unchanged, and neither desktop nor compact layouts overflow.

Criterion-evidence adapters must preserve exact criterion order and use explicit check/probe/journey mappings. Tests must show unsupported criteria fail before Chromium launch, multi-step journeys reload into an isolated start state, every step passes before criterion evidence is emitted, and every evidence row references retained validation proof. Worker result-contract proof must reject entrypoints missing planner-required semantic selectors before submitting a candidate to browser validation. Output revision proof must start from **Ask Soma for changes**, retain trusted output/team/run/work/digest context, reject stale ownership, preserve the original package, inherit its acceptance contract, and validate a separate revision target.
...
Use `--server-mode=start` for production-start proof and `--live-backend` when the spec must hit a real Core backend. Invoke manages server lifecycle, serial workers, and cleanup. Playwright browser binaries live under the managed project cache and receive a disk-derived quota; Interface install and E2E fail before browser downloads or execution when the filesystem reserve, aggregate cache budget, or Playwright share is exceeded. Review `uv run inv cache.status`, then use `uv run inv cache.clean` for managed tool caches or `uv run inv clean.disk-status` followed by `uv run inv clean.generated` for the broader allowlisted repo-local residue set. Neither cleanup path owns `.env`, whole runtime workspaces, Compose data, or Docker volumes.
...
uv run inv compose.storage-health
```
The schema compatibility probe must verify the current `001_current_schema.sql` baseline, including late tables and constraints: an empty public schema installs once, a fully compatible retained schema is a row-preserving no-op, and a partial or incompatible nonempty schema fails before SQL execution. There is no executable development downgrade chain; retained data requires a backup plus a supported upgrade path or an intentional reset.
## CI Pipelines
...
Release-preflight lane presets are `baseline`, `runtime`, `service`, and `release`; use `uv run inv ci.release-preflight --lane=release` for the full release gate. Before production Interface builds, the managed lifecycle stops repo-owned servers and Playwright listeners, verifies service ownership before relying on occupied ports, and clears `.next` so a running Windows process cannot lock the candidate output. Windows process inventory uses one bounded repo-scoped Node/cmd query before tree termination, and Windows process-tree termination uses a bounded extended wait because loaded hosts may take close to a minute to enumerate Next.js descendants; falling back early can stop only the parent and leave a child holding the build directory. The baseline browser stage runs the managed, repo-provisioned Chromium project with one worker against the production Interface build and retains JSON/JUnit evidence. Cross-engine proof is a separately provisioned matrix and is not implied by the default install. Every preset begins with `ci.lint`, and a lint failure prevents baseline, service, or deployment-facing stages from running. CI Go lint/test commands use `go -C <core>` so the Core module is explicit even when Invoke or the host shell does not preserve a directory context; release baseline Core tests add `-p 1` because the package set includes local runtime/browser probes that must not race package-level resource contention; failed hidden commands must print their captured output after escaping characters that the active host console cannot encode.

Core tests that assert an unconfigured provider posture must clear all `MYCELIS_PROVIDER_*` and `MYCELIS_PROFILE_*` overrides within the test. Identity tests that assert a specific local owner must likewise set `MYCELIS_LOCAL_ADMIN_USERNAME` and `MYCELIS_LOCAL_ADMIN_USER_ID` explicitly. Release tasks intentionally load repo-local configuration, so a test may not assume the parent process has empty or default provider and identity values.
...
- Playwright for operator-visible workflows and live-backend proxy/Core/runtime contracts
