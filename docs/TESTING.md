...
Use `npm test -- WorkspaceExplorer MCPToolRegistry MCPLibraryBrowser MCPServerCard --maxWorkers=1` from `interface/` when Capabilities, filesystem MCP, Output Files browsing/open-folder behavior, or MCP output generation contracts change. Use `npm test -- DeploymentContextPanel ResourcesPage --maxWorkers=1` when Resources -> Deployment Context intake compression changes; proof should verify the content/classification/scope tabs, bounded loaded-context list, and unchanged POST payloads.
