...
- primary controls remain physically reachable in the active browser window after dynamic state appears; for Dashboard/Soma this includes clicking and filling the composer with current-work/output lanes visible, then verifying the hit target and bounding box stay inside the viewport rather than relying on selector presence alone
- Soma dashboard result cards stay compact: trusted results render as chat receipts with one obvious latest-output open path and small media preview; the current-work lane shows current workflow, active task posture, latest output access, and next action when reviewable work exists; failed/degraded results keep visible boundaries; full output lists, proof detail, capability usage, logs, and event history stay in Work panel, Runs, Activity, Resources, or Inspect surfaces.
...
| TeamWorkItem/TeamInteraction/TeamStatusEvent | Go/API proof that team creation remains non-active (`new`/`briefed`), delegated work queues real work, retained deliverables move through queued/running/output-ready or degraded states, async bounded `work/ask` calls persist queued Active Work transactionally, record durable dispatch tracking before publishing, publish and flush governed command envelopes without blocking the browser, record a running dispatch event/interaction with subject/channel plus recovery-deadline hints, and keep overlapping plain team responses correlated FIFO unless the response carries an explicit `work_item_id`, offline/publish/flush failures persist `degraded` proof immediately, legacy synchronous asks still persist either `output_ready` readable team replies with retained `text_reply` output/proof refs or `degraded` timeout/offline/unreadable-response proof, direct ask dispatch/output/degraded lifecycle writes status event, mission event, work-item update, and interaction transactionally, `start_work`/`pause`/`resume`/`archive`/`steer`/`recover` action transitions persist status and interaction evidence, `include_archived=false` hides cleared rows from active review reads, run/contract/proof/output refs persist where available, durable `output_refs[].storage_ref` remains a workspace path rather than a viewer URL, and run-linked status transitions emit `team_work.status` mission events for Event Spine reconstruction | `IN_REVIEW` for confirm-action wiring, async bounded ask dispatch, action endpoint, live team-output API readback, and Event Spine bridge |
...
Related active contracts: [V8.3 Operational Embodiment PRD](architecture-library/V8_3_OPERATIONAL_EMBODIMENT_PRD.md), [V8.3 Soma User Experience Contract](architecture-library/V8_3_SOMA_USER_EXPERIENCE_CONTRACT.md), [V8.3 MVP UI Runtime Delivery Plan](architecture-library/V8_3_MVP_UI_RUNTIME_DELIVERY_PLAN.md), [V8 New-User Acceptance Matrix](architecture-library/V8_NEW_USER_ACCEPTANCE_MATRIX.md), [V8 UI/API and Operator Experience Contract](architecture-library/V8_UI_API_AND_OPERATOR_EXPERIENCE_CONTRACT.md), [V8.2 Soma Team Interaction Contract](architecture-library/V8_2_SOMA_TEAM_INTERACTION_CONTRACT.md), and [V8 UI Testing Product Contract](architecture-library/V8_UI_TESTING_AGENTRY_PRODUCT_CONTRACT.md). Current GUI proof status: live Soma governance/team/playable-output flows are green; `team-output-content-live.spec.ts` proves chat -> confirm-action -> durable team work -> status events -> reveal API readback -> focused Dashboard output digest -> Groups Workflow Log review -> Resources folder readback; unit proof covers proof-linked approval feedback, current-work lane summary, latest-output file/folder guidance, focused Dashboard review-inbox recovery cards, full Teams list-detail review inbox, and closed-panel dashboard compression; `desktop-mobile-compression.spec.ts` now includes a short-viewport Dashboard composer reachability guard plus the Dashboard `Review work` focused-inbox panel that checks readability, queue summary, `Open inbox`, and recovery/clear primary actions; `teams.spec.ts` covers `/teams?view=work` list-detail review with compact rows, selected detail, and recovery/output actions. Legacy `v8-organization-entry.*` setup specs describe the removed dashboard setup affordance and are skipped in the default broad browser suite until they are rewritten around the Soma-first dashboard entry.
...
- Green focused proof includes signed-in `/login` -> `/dashboard` entry, Soma governance, Resources Output Files, Capabilities, Settings, Accessibility baseline, homepage/navigation/layout/package/team/groups/system/mobile/compression, and the new-user acceptance surfaces in [V8 New-User Acceptance Matrix](architecture-library/V8_NEW_USER_ACCEPTANCE_MATRIX.md). Dense-panel proof uses `clickVisibleControl` in `interface/e2e/support/click-visible-control.ts`, Settings/Auth follows the Advanced Settings -> Auth Providers and Resources -> Capabilities contract, and the accessibility baseline keeps critical checks active while `color-contrast` remains split out after axe timeouts in headed Chromium.
