...
- **Live Backend Mode:** `uv run inv interface.e2e --live-backend --server-mode=start ...` loads topology from `.env.compose` and credentials from `.env`, then enables specs that require a real Core backend
