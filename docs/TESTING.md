...
Core tests that assert an unconfigured provider posture must clear all `MYCELIS_PROVIDER_*` and `MYCELIS_PROFILE_*` overrides within the test. Release tasks intentionally load repo-local configuration, so a test may not assume the parent process has an empty provider environment.
