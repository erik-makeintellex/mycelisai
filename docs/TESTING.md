...
- Worker Profile changes must prove seeded built-ins, locked CRUD behavior, runtime `profile_ref` hydration and missing-ref failure, capability/context payloads, and a live copy/edit/delete lifecycle. Rebuild `core/bin/server.exe` before external live proof; restarting a stale binary is not proof.
