...
- in the supported home-runtime stack, `.env` is the local secret store and `.env.compose` must keep container-host assumptions separate; use `MYCELIS_COMPOSE_OLLAMA_HOST` in `.env.compose`, and keep it container-reachable instead of `localhost`, `127.0.0.1`, or `0.0.0.0`
...
- when testing governed search in Compose or Helm, use `MYCELIS_SEARCH_PROVIDER`, `MYCELIS_SEARXNG_ENDPOINT`, `MYCELIS_SEARCH_LOCAL_API_ENDPOINT`, and `MYCELIS_SEARCH_MAX_RESULTS` as deployment-time runtime knobs; the supported Compose release path starts SearXNG by default for Brave-free public web search, while `local_sources` and self-hosted `local_api` remain valid alternate postures, but provider selection alone is not a health proof for a reachable endpoint
...
| `/settings` (+ `/settings/tools`) | `SettingsPage.test.tsx`, settings component suites, `AuthProvidersPage.test.tsx`, `MCPToolRegistry.test.tsx`, `MCPLibraryBrowser.test.tsx` | `settings.spec.ts` for guided settings/profile/access/theme/Auth Providers; Connected Tools browser depth now lives on `/resources?tab=tools` in `mcp-connected-tools.spec.ts` | `ACTIVE` |
...
| `interface/e2e/specs/settings.spec.ts` | Settings guided setup, profile/access/theme persistence, and Advanced-mode Auth Providers scaffold; Connected Tools MCP browser proof now lives in `mcp-connected-tools.spec.ts` |
...
- `interface/e2e/specs/soma-media-artifacts.spec.ts`: mocked image/audio/file/document artifacts, visible `Artifact result`, save action, saved path, and download references.
...
- **Task Cleanup:** `uv run inv interface.e2e` stops any stale listener on `:3000` before and after each run, launches a managed local Next.js server, binds that managed server to `127.0.0.1` on Windows for stable start-mode proof, defaults Playwright to `--workers=1`, sweeps repo-local Next/Vitest/Playwright worker residue, clears an orphaned `interface/.next/dev/lock` only when no repo-local Next worker remains, and fails closed if the managed server exits or a stale port prevents it from owning the browser target
- **Managed Browsers:** default task runs expect Playwright browser binaries under `workspace/tool-cache/playwright`
- **Live Backend Mode:** `uv run inv interface.e2e --live-backend --server-mode=start ...` loads topology from `.env.compose` and credentials from `.env`, then enables specs that require a real Core backend. `soma-governance-live.spec.ts` allows a longer first chat response window because the first cold local-model answer in WSL/Compose can be slower than warmed follow-up requests.
