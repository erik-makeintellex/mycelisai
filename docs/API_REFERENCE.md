...
| `/api/v1/teams/{id}/work/{workItemId}/actions` | POST | Apply an audited operator control to a durable work item. Body uses `action=start_work|pause|resume|archive` with optional `summary`, `actor_ref`, `source_kind`, `source_channel`, `payload_kind`, `payload`, and `audit_refs`. The endpoint validates transitions, rejects `create_team` shell records, writes a `TeamStatusEvent`, writes a `TeamInteraction`, updates the work item state/last event, and returns the updated `TeamWorkItem`. Current transitions: `new|briefed|queued -> running` for `start_work`, `queued|running|needs_operator|reviewing|degraded -> paused`, `paused -> queued` for `resume`, and non-archived delegated/deliverable work -> `archived`. |
