...
| `/api/v1/search/status` | GET | Current Mycelis Search provider posture for UI/Soma capability answers, including provider, configured/enabled flags, direct `web_search` support, token requirements, online-allowed/no-confirm disclosure posture, and blocker/next-action copy. Built-in `web_search` is independent of the optional `fetch` MCP, which is used for explicit URL retrieval when configured. |
