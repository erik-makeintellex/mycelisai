...
> **Status:** ACTIVE — Team C (Scheduler) is NEXT; Team B (Trigger Engine) is COMPLETE
...
> | Team E | Causal Chain UI — ViewChain.tsx, RunChainNode.tsx, /runs/[id]/chain page | 🔲 REQUIRED |
> | MCP Baseline | filesystem, memory, artifact-renderer, fetch MCP servers | 🔲 REQUIRED |
> | Cross-Cut | Soma/Council engagement protocol (internal vs MCP vs API vs code-loop pathing) | ✅ COMPLETE (prompt + architecture contract) |
...
> | Cross-Cut | Root-admin full configuration contract (Soma executes beyond team-only flows) | ✅ COMPLETE |
> | Runtime Ops | Cognitive startup probe scoping (`ollama` default + profile-routed providers only) | ✅ COMPLETE |
> | Cross-Cut | NATS signal standardization (`signal.status`/`signal.result`/source-kind rules + dev-only infrastructure channel exclusion) | ✅ COMPLETE |
> | Cross-Cut | UI target + transaction contract (terminal states, backend effects, product-flow proof) | ✅ COMPLETE |
...
|------|-------|-------|-------------|-------------------|
| **A — Event Spine** | Migrations 023-024, MissionEventEnvelope struct, event store, run manager, API handlers, CTS extension, NATS topics, wiring into Soma/Agent, degraded mode | 9 major steps | None (foundational) | `uv run inv core.test` passes, GET /runs/{id}/events returns events, GET /runs/{id}/chain returns tree |
| **B — Trigger Engine** | Migrations 025-026, trigger store with cache, evaluation engine with 3 guards, trigger CRUD APIs, wiring into event ingest | 5 major steps | Team A complete (uses mission_events, mission_runs, events.Store, runs.Manager) | `uv run inv core.test` passes, trigger rules CRUD works, mission.completed fires child run |
| **C — Scheduler** | Migration 027, cron scheduler goroutine, schedule CRUD + pause/resume APIs, NATS disconnect/reconnect wiring | 4 major steps | Team A complete (uses runs.Manager, events.Store) | `uv run inv core.test` passes, schedule creates run on tick, suspends on NATS disconnect |
| **E — Run Timeline UI** | RunTimeline, ViewChain, EventCard, RunChainNode components, Zustand slices, route pages, MissionControl integration | 5 major steps | Team A APIs available (GET /runs/*/events, GET /runs/*/chain) | `uv run inv interface.build` succeeds, timeline renders events, chain renders tree |
| **D — Navigation** | 3 new pages (Automations, Resources, System), nav restructure, degraded mode messaging, content migration from old routes | 7 major steps | Team E complete (Automations page needs run timeline integration) | `uv run inv interface.build` succeeds, all 5 nav items reachable, old routes redirect |
...
- [ ] Migrations 023-024 apply cleanly (`uv run inv core.test` includes migration test)
...
- [ ] `uv run inv interface.build` succeeds
