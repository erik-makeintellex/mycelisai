...
| P0.9 | Full journey proof | COMPLETE | Ask through Revisit | Headed and headless proof cover ask, understand, approve, execute, deliver, trust, recover, and revisit. The source-stack certification uses Soma, governed approval, NATS team execution, a local model, retained project-package output, browser interaction, proof/run readback, and Dashboard/Resources/Groups/Activity revisit; the broader GUI sweep covers primary/admin routes and critical workflows with no blocking console/page errors or horizontal overflow. |
