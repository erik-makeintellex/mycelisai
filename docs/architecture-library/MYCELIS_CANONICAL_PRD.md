...
| P0.9 | Full journey proof | IN_REVIEW | Ask through Revisit | Existing headed and headless proof covers the runtime journey and broad route mechanics. Acceptance still requires an untouched first viewport with one clear Ask/work/deliverable path, continued conversational steering during delayed work, concise asynchronous completion with one primary output action, a dedicated substantial-output surface whose Back action restores context, no default runtime vocabulary, and manual novice comprehension proof. |
