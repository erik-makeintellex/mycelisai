...
| P0.4 | Bus handoff and started feedback | IN_REVIEW | Execute, Recover | Approval or quick action immediately creates visible started state, correlation, durable work linkage, expected-output/proof context in run-linked events, and actionable degradation when NATS/team dispatch or retained-output completion cannot complete. |
