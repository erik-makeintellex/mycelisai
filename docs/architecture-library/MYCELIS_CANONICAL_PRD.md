...
| P0.9 | Full journey proof | ACTIVE | Ask through Revisit | Existing headed and headless proof covers the runtime journey and broad route mechanics. The dedicated substantial-output canvas, one primary conversational completion action, safe retained-file boundary, and exact Back-to-Soma context contract pass focused and headed desktop/compact interaction, overflow, and error proof. Active run/team/work identity now remains attached to the composer; explicit guidance commits atomically and enters the team's canonical NATS command path while ordinary questions remain normal conversation. Acceptance still requires headed genuinely delayed-work proof, no default runtime vocabulary, and manual novice comprehension proof. |
