...
| P0.7a | Search and data-source registry | BLOCKED | Ask, Trust | Search status and Resources show public web, approved local/mounted data, private/API sources, and named mounts; persisted sources carry endpoint/path, scope, boundary, auth or mount mode, secret-ref when needed, sensitivity, trust, and recovery metadata; mixed local-plus-web asks state coverage clearly. Current acceptance blocker: the authenticated Core source-registry endpoint is healthy, but the Interface source-management route returns HTTP 500 and leaves the Access lane unavailable. |
