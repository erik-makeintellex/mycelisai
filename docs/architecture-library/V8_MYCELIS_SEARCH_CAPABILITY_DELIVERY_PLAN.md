...
- Soma can search the public web through a self-hosted SearXNG option when the operator enables that service.
...
| --- | --- | --- | --- |
| Architecture Lead | ACTIVE | Own provider contract, trust boundaries, API shape, and promotion rules. | `SearchProvider` contract covering `local_sources`, `searxng`, `brave`, and `disabled`. |
...
| Ops/Runtime Delivery | ACTIVE | Add self-hosted SearXNG as an optional Compose/Kubernetes profile. | `MYCELIS_SEARCH_PROVIDER`, `MYCELIS_SEARXNG_ENDPOINT`, and service-health checks. |
...
- snippet
- source_kind: local_source | searxng | brave | hosted_search
- trust_class
- sensitivity
- retrieved_at
- provider_metadata
...
Runtime tests:
- provider config resolves `disabled`, `local_sources`, `searxng`, and `brave` deterministically.
