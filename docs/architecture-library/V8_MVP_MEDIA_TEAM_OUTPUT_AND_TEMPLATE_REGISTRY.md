...
- direct-vs-team proof: one prompt returns a direct inline Soma answer, while a deliverable-package prompt produces a team-managed output contract. `interface/e2e/specs/workflow-output.direct.spec.ts`, `interface/e2e/specs/workflow-output.compact-team.spec.ts`, `interface/e2e/specs/workflow-output.multi-lane.spec.ts`, and `interface/e2e/specs/workflow-output.reload-review.spec.ts` now cover this mocked browser proof set.
- media proof: image request either returns a real image artifact from the configured local/self-hosted or hosted media provider, or a clear missing-engine / missing-credential / approval-needed blocker with next setup action. `interface/e2e/specs/soma-media-artifacts.spec.ts` now covers preview/save/download rendering with generated media artifact payloads, while `interface/e2e/specs/v8-ui-testing-agentry.spec.ts` remains the continuity-focused operator flow around that lane; live configured-provider proof remains separate.
...
- MCP proof: a team workflow uses an MCP-backed capability and the operator can see which MCP service/tool was used in the connected-tools/activity surface. `interface/e2e/specs/mcp-connected-tools.spec.ts` covers the Connected Tools browser side for persisted activity, expanded server tools, curated install, visible team/agent/run correlation metadata, and a live-gated temporary team-lane correlation path; the live path still needs execution against a healthy backend with filesystem MCP available.
...
6. `IN_REVIEW` add MCP usage proof tied to connected-tools activity; browser proof now covers the Connected Tools side plus a live-gated team-lane correlation path, and the remaining validation step is to run that live path with `PLAYWRIGHT_LIVE_BACKEND`.
