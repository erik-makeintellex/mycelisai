...
- MCP proof: a team workflow uses an MCP-backed capability and the operator can see which MCP service/tool was used in the connected-tools/activity surface. `interface/e2e/specs/mcp-connected-tools.spec.ts` covers the Connected Tools browser side for persisted activity, expanded server tools, curated install, visible team/agent/run correlation metadata, and the live-gated temporary team-lane correlation path; the live path passed from the WSL deployment-mimic checkout with filesystem MCP available.
...
6. `COMPLETE` add MCP usage proof tied to connected-tools activity; browser proof now covers the Connected Tools side plus a live-gated team-lane correlation path, and the live path has passed with `PLAYWRIGHT_LIVE_BACKEND`.
