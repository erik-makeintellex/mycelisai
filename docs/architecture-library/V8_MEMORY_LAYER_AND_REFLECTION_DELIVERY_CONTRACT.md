...
> Purpose: Define the explicit memory layer taxonomy, promotion rules, and team delivery architecture for reflection-first learning without weakening governance.
...
| `AGENT_MEMORY` | Team or specialist continuity, decisions, role-specific observations, and execution lessons. | Scoped durable memory rows/vectors with `team_id`, `agent_id`, and visibility metadata. | Team/agent scoped by default. Wider promotion requires review. |
...
- `AGENT_MEMORY` must not become organization-wide behavior unless promoted.
...
| --- | --- | --- |
| Memory Architecture Lead | Layer taxonomy and promotion model | Keep `SOMA_MEMORY`, `AGENT_MEMORY`, `PROJECT_MEMORY`, and `REFLECTION_MEMORY` explicit in docs, prompts, API contracts, and tests. |
...
| Runtime/Backend Lead | Tool and persistence contracts | Add promotion APIs/tools only after candidate review semantics are stable. Maintain scoped retrieval boundaries. |
...
4. `NEXT`: add a promotion command that consumes a reviewed exchange candidate and writes to the correct layer.
5. `NEXT`: add advanced UI review for learning candidates, confidence, evidence, and promotion target.
6. `NEXT`: add browser proof for a reflection candidate that is captured, reviewed, promoted, and then recalled from `reflection_synthesis`.
