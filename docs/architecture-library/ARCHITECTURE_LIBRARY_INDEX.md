...
| [V8 Governed Deployment Context and RAG Plan](V8_GOVERNED_DEPLOYMENT_CONTEXT_AND_RAG_PLAN.md) | operationalizing the separate customer-context and company-knowledge pgvector stores for Soma, Council, and teams | canonical plan for governed deployment-context ingest, promotion, policy-bound web/MCP intake, retrieval boundaries, and release proof |
| [V8 Structured Team Asks and Lane Routing Plan](V8_STRUCTURED_TEAM_ASKS_AND_LANE_ROUTING_PLAN.md) | defining one canonical structured delegation contract from Soma to teams and the phased path toward lane-role-aware team routing | canonical plan for typed `TeamAsk` payloads, backward-compatible `delegate_task` normalization, team command-bus preservation, later agent specialization, and instantiation defaults |
...
| [V8 Structured Team Asks and Lane Routing Plan](V8_STRUCTURED_TEAM_ASKS_AND_LANE_ROUTING_PLAN.md) | coordinating one shared machine-readable contract for Soma-to-team asks, lane roles, command-lane preservation, and future team-instantiation defaults | canonical plan for typed team asks, reusable lane-role vocabulary, routed delegation proof, and later UI/operator exposure |
...
22. Use [V8 Governed Deployment Context and RAG Plan](V8_GOVERNED_DEPLOYMENT_CONTEXT_AND_RAG_PLAN.md) when operationalizing the separate `customer_context` and `company_knowledge` pgvector stores, their promotion rules, and policy-bound external/web ingest posture.
23. Use [V8 Structured Team Asks and Lane Routing Plan](V8_STRUCTURED_TEAM_ASKS_AND_LANE_ROUTING_PLAN.md) when defining the canonical Soma-to-team delegation contract, the first typed `TeamAsk` slice, and the later routing/instantiation phases for lane-aware team execution.
24. Use [V8 Content Generation And Collaboration Strike Team Plan](V8_CONTENT_GENERATION_AND_COLLABORATION_STRIKE_TEAM_PLAN.md) when coordinating content delivery posture, generated artifact proof, media/file generation behavior, and policy-configurable specialist/model collaboration.
25. Use [V8 Approval and Product Trust Strike Team Plan](V8_APPROVAL_AND_PRODUCT_TRUST_STRIKE_TEAM_PLAN.md) when coordinating user-first proposal surfaces, content/artifact value delivery, details-on-demand governance metadata, and release acceptance for approval UX changes.
26. Use [V8 Universal Soma And Context Model PRD](V8_UNIVERSAL_SOMA_AND_CONTEXT_MODEL_PRD.md) when correcting product identity around one persistent Soma/Council pair, Central Soma home, context switching, scoped execution, and the transition away from org-scoped Soma language.
27. Use [V8 Home Docker Compose Runtime](V8_HOME_DOCKER_COMPOSE_RUNTIME.md) when supporting the single-host Docker Compose runtime for home-lab, demo, and partner-review use without losing observability or compose-task discipline.
28. Use [V8 True MVP Finish Plan](V8_TRUE_MVP_FINISH_PLAN.md) when coordinating the final MVP push across product value delivery, settings completion, demo readiness, and release proof.
29. Use [V8 Demo Product Strike Team Plan](V8_DEMO_PRODUCT_STRIKE_TEAM_PLAN.md) when coordinating product simplification, demo readiness, feature preservation, workflow proof, and partner/funder-facing legibility.
30. Use [V8 Demo Product Execution Brief](V8_DEMO_PRODUCT_EXECUTION_BRIEF.md) when executing the first engaged-team deliverables for default-vs-advanced surfaces, feature preservation, the golden-path partner demo, and the product-focused UI verification checklist.
31. Use [V8 Demo Product Wording Drift Inventory](V8_DEMO_PRODUCT_WORDING_DRIFT_INVENTORY.md) when aligning README, landing, organization setup, organization home, and retention/governance language to the current product story without blindly renaming deep internal contracts.
32. Use [V8 Demo Product Feature Retention Map](V8_DEMO_PRODUCT_FEATURE_RETENTION_MAP.md) when proving that advanced routes, governed mutation depth, memory inspection, system diagnostics, and broadcast/specialist power remain reachable while the default demo story stays simple.
33. Use [V8 MVP Investor Demo and Governed Capability Plan](V8_MVP_INVESTOR_DEMO_AND_GOVERNED_CAPABILITY_PLAN.md) when preparing an investor-ready story across Soma’s initial value, governed mutation, MCP-powered input/output, securable web research, and inspectable context-security controls.
34. Use [V8 Partner Demo Script](V8_PARTNER_DEMO_SCRIPT.md) when preparing the canonical technical-partner/funder demonstration and keeping the story centered on product value rather than infrastructure depth.
35. Use [V8 Partner Demo Verification Checklist](V8_PARTNER_DEMO_VERIFICATION_CHECKLIST.md) when the UI testing and release teams need to prove that the partner demo shows product value, governed trust, structure, continuity, preserved advanced depth, and governable external capability.
36. Use [V8.2 Cross-Repo Cleanup and Release Structure Plan](V8_2_CROSS_REPO_CLEANUP_AND_RELEASE_STRUCTURE_PLAN.md) when coordinating git hygiene, commit boundaries, cleanup lanes, release structure, and final validation order across a mixed worktree.
37. Use [V8 Release Platform Review: Security, Monitoring, and Debug](V8_RELEASE_PLATFORM_REVIEW_SECURITY_MONITORING_DEBUG.md) when aligning release truth across security/governance, monitoring/ops, and debug/live-browser proof.
38. Use [V8 Dev Team Execution and UI Engagement Plan](V8_DEV_TEAM_EXECUTION_AND_UI_ENGAGEMENT_PLAN.md) when coordinating cross-team execution order, dev-lane ownership, UI/browser proof, engagement testing, and committed-slice discipline for the active MVP push.
39. Use [V8 Ask-Class, Agent-Type, and Output Contract Plan](V8_ASK_CLASS_AGENT_TYPE_OUTPUT_CONTRACT_PLAN.md) when consolidating ask classification, specialist targeting, runtime output assertion, and shared test proof across Soma, council, and governed execution surfaces.
40. Use [V8 Structured Team Asks and Lane Routing Plan](V8_STRUCTURED_TEAM_ASKS_AND_LANE_ROUTING_PLAN.md) when defining Soma-to-team ask payloads, lane-role vocabulary, command-lane preservation, and the first reusable delegation contract for future team-instantiation work.
41. Use [V8 User Workflow Execution and Verification Plan](V8_USER_WORKFLOW_EXECUTION_AND_VERIFICATION_PLAN.md) when validating that Mycelis supports user-shaped delivery, standards-compliant MCP usability, and workflow-complete operator outcomes rather than only interaction-level success.
42. Use [V8 User Intent Workflow Review and Instantiation Plan](V8_USER_INTENT_WORKFLOW_REVIEW_AND_INSTANTIATION_PLAN.md) when deciding whether an ask should stay direct, instantiate a managed team, or instantiate an external workflow contract such as `n8n`, and when defining proof for target-output delivery such as bounded image generation.
43. Use [V8 Full Testing Action Plan](V8_FULL_TESTING_ACTION_PLAN.md) when you need one ordered release-style run across repo baseline, live service proof, compose-aware runtime proof, focused workflow suites, and final docs/state verification.
44. Use [V8.2 Full Production Architecture](../../v8-2.md) when checking whether a surface belongs to the canonical full production, managed exchange, and actuation target rather than the current release.
