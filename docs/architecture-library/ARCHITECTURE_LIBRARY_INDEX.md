...
31. Use [V8 Partner Demo Script](V8_PARTNER_DEMO_SCRIPT.md) when preparing the canonical technical-partner/funder demonstration and keeping the story centered on product value rather than infrastructure depth.
...
36. Use [V8 Ask-Class, Agent-Type, and Output Contract Plan](V8_ASK_CLASS_AGENT_TYPE_OUTPUT_CONTRACT_PLAN.md) when consolidating ask classification, specialist targeting, runtime output assertion, and shared test proof across Soma, council, and governed execution surfaces.
37. Use [V8 User Workflow Execution and Verification Plan](V8_USER_WORKFLOW_EXECUTION_AND_VERIFICATION_PLAN.md) when validating that Mycelis supports user-shaped delivery, standards-compliant MCP usability, and workflow-complete operator outcomes rather than only interaction-level success.
38. Use [V8 User Intent Workflow Review and Instantiation Plan](V8_USER_INTENT_WORKFLOW_REVIEW_AND_INSTANTIATION_PLAN.md) when deciding whether an ask should stay direct, instantiate a managed team, or instantiate an external workflow contract such as `n8n`, and when defining proof for target-output delivery such as bounded image generation.
39. Use [V8 Full Testing Action Plan](V8_FULL_TESTING_ACTION_PLAN.md) when you need one ordered release-style run across repo baseline, live service proof, compose-aware runtime proof, focused workflow suites, and final docs/state verification.
40. Use [V8.2 Full Production Architecture](../../v8-2.md) when checking whether a surface belongs to the canonical full production, managed exchange, and actuation target rather than the current release.
