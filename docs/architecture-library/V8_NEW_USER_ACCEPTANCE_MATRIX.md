...
| Dashboard orientation | Review `/dashboard` before asking anything | Soma expression input is primary; Trust does not imply prior work; the compact current-work lane shows the current workflow, active task posture, latest output access, and next action only when there is work to review; Active Work is attention-first and capped, with full backlog linked to `/teams`; Output and Context explain where retained results and setup evidence will appear. |
| Setup checklist | Review setup surfaces before a demo | Operator can identify provider/search posture, Capabilities/MCP posture, workspace root, artifact/output root, and deployment trust route without reading logs. |
...
| MCP readiness | Open `Resources -> Capabilities`, choose **Add MCP Server** when needed | Curated library path is visible; required env vars are named without secret values; capability state and recent MCP activity confirm what Soma and teams can use. |
...
uv run inv interface.e2e --headed --project=chromium --workers=1 --spec=e2e/specs/focused-team-output-dashboard.spec.ts
