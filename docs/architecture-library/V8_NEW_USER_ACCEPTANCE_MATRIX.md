...
| Workspace/output roots | Open `Resources -> Workspace Files` and `System -> Deployments` when file output is expected | `MYCELIS_WORKSPACE` and `MYCELIS_ARTIFACT_ROOT` are identifiable; filesystem MCP writes and project packages stay inside the governed workspace boundary. |
