...
async function mockGroupsWorkspace(page: Page) {
    ...
    const artifactsByTeam = new Map<string, ArtifactRecord[]>([
        ['launch-lead', [
            {
                id: 'artifact-brief',
                team_id: 'launch-lead',
    ...
    await page.route(/\/api\/v1\/artifacts\?team_id=.*/, async (route) => {
        const url = new URL(route.request().url());
        const teamId = url.searchParams.get('team_id') ?? '';
        await fulfillJson(route, 200, { ok: true, data: artifactsByTeam.get(teamId) ?? [] });
    });
...
test.describe('Groups workspace (/groups)', () => {
    test('separates standing and temporary groups and keeps recent outputs reviewable', async ({ page }) => {
        ...
        await expect(page.getByText('Standing groups')).toBeVisible();
        await expect(page.getByText('Temporary groups')).toBeVisible();
        ...
        await expect(page.getByText('Asset Bundle')).toBeVisible();
        await expect(page.getByText('workspace/groups/temporary-launch-sprint/assets.zip')).toBeVisible();
    ...
    test('creates a temporary group and supports broadcast workflow for business execution', async ({ page }) => {
        ...
        await expect(page.getByRole('button', { name: /Regional Expansion Sprint Prepare outreach, pricing notes, and operator-ready launch assets for a new region\./ })).toBeVisible();
