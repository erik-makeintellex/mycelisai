...
test.describe('Settings Page (/settings)', () => {
    ...
    test('People & Access keeps enterprise user management out of the base release path', async ({ page }) => {
        ...
        await expect(page.getByRole('button', { name: /^Self-hosted release\b/i })).toHaveCount(0);
        await expect(page.getByRole('button', { name: /^Self-hosted enterprise\b/i })).toHaveCount(0);
        await expect(page.getByRole('button', { name: /^Hosted control plane\b/i })).toHaveCount(0);
        await expect(page.getByRole('combobox', { name: 'Identity Mode' })).toHaveCount(0);
        await expect(page.getByLabel('Identity Mode')).toHaveText('Local only');
        await expect(page.getByLabel('Shared Agent Specificity Owner')).toHaveText('Root admin only');
        await expect(page.getByTestId('deployment-access-ownership')).toHaveText('Deployment-owned');
