...
test.describe("Live Soma P0 browser game delivery", () => {
  ...
  test("creates and opens a playable Zelda-like browser game through Soma", async ({ page }) => {
    test.slow();
    const stamp = Date.now();
    const teamID = `p0-zelda-game-${stamp}`;
    const title = "P0 Zelda Browser Game Team First Playable";
    ...
    const proposal = await submitLiveWorkspaceChat(page, [
      `Create a team with team_id ${teamID} named P0 Zelda Browser Game Team.`,
      "Ask Soma and that team to build a playable Zelda-like browser game project package.",
      "Use only code-generated browser graphics, no external assets, and keep it small enough to inspect.",
      "It must include movement, collision, enemies or hazards, health, key pickup, locked door, win state, fail state, and restart.",
    ...
    expect(projectPackage?.validation).toMatch(/movement|collision|hazards|key|door|win\/fail|restart/i);
    ...
    await expect(page.getByText("P0 Zelda Browser Game Team").last()).toBeVisible();
    ...
    await page.getByRole("button", { name: /Open file P0 Zelda Browser Game Team/i }).last().click();
    ...
    await expect(gamePage.locator("#health")).toHaveText("3");
