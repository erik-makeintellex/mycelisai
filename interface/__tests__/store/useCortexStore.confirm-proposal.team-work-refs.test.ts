...
describe("useCortexStore confirm proposal team work refs", () => {
    ...
    it("labels degraded confirmation refs as recovery work", async () => {
        useCortexStore.setState({
            pendingProposal: {
                intent: "Launch a build crew",
                teams: 1,
        ...
        mockFetch.mockResolvedValueOnce({
            ...
            json: async () => ({
                data: {
                    run_id: "run-work-123456",
                    team_work_refs: [{
                        work_item_id: "work-beta-123456",
                        state: "degraded",
                        run_id: "run-work-123456",
                        expected_outputs: ["Playable browser app"],
                    }],
                },
            }),
        }).mockResolvedValueOnce({ ok: true, json: async () => ([]) });
        ...
        const message = useCortexStore.getState().missionChat.at(-1);
        expect(message?.content).toContain("Work work-bet needs recovery.");
        expect(message?.content).toContain("Expected output: Playable browser app.");
        expect(message?.content).toContain("Review Current Work for recovery.");
    });
});
