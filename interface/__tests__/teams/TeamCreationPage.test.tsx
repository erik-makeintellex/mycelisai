...
describe("TeamCreationPage", () => {
  ...
  it("loads the current organization and runs the guided Soma creation workflow", async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          ...
          data: {
            ...
            priority_steps: [
              "Confirm campaign outcomes",
              "Define owned deliverables",
            ],
    ...
    expect(
      screen.getByRole("button", { name: "Marketing launch team" }),
    ).toBeDefined();
    expect(screen.getByText(/Default team shape/i)).toBeDefined();
    expect(screen.getByText(/Start with the team lead only/i)).toBeDefined();
    expect(
      screen.getByText(
        /missing capability, owned task, expected proof, and removal point/i,
      ),
    ).toBeDefined();
    ...
    expect(
      await screen.findByText("Launch team plan for Northstar Labs"),
    ).toBeDefined();
  });
  ...
  it("shows recovery guidance when no organization is available yet", async () => {
    ...
    expect(
      screen.getByText("Choose or create an AI Organization first"),
    ).toBeDefined();
    expect(
      screen
        .getAllByRole("link", { name: "Open Soma workspace" })[0]
        .getAttribute("href"),
    ).toBe("/dashboard");
