...
const MOCK_ARTIFACT: CTSEnvelope = {
    id: 'art-1',
    source: 'agent-alpha',
    signal: 'artifact',
...
describe('GovernanceModal', () => {
    ...
    it('renders when selectedArtifact is set', () => {
        useCortexStore.setState({ selectedArtifact: MOCK_ARTIFACT });
    ...
    it('shows "No proof of work" when proof is null', () => {
        const noProof = { ...MOCK_ARTIFACT };
        delete noProof.proof;
        useCortexStore.setState({ selectedArtifact: noProof });
        render(<GovernanceModal />);
    ...
    it('calls selectArtifact(null) on close button click', () => {
        useCortexStore.setState({
            selectedArtifact: MOCK_ARTIFACT,
        });
        render(<GovernanceModal />);
