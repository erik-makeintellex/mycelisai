                ...
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm font-semibold text-cortex-text-main">
                            {availableCount} available capability{availableCount === 1 ? "" : "ies"}
                        </p>
...
function CapabilityCard({ capability }: { capability: CapabilityManifest }) {
    ...
    const boundServer = capability.bound_server_name ?? capability.bound_server_id ?? capability.server_or_package;
    const boundTool = capability.bound_tool_name ?? capability.bound_tool_id;
    ...
    return (
        <div className="rounded-lg border border-cortex-border bg-cortex-bg/60 px-3 py-3">
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-3">
                ...
                <CapabilityDetail label="Bound provider/server/tool" value={[capability.provider, boundServer, boundTool].filter(Boolean).join(" / ") || capability.source} />
            </div>
        </div>
    );
}
