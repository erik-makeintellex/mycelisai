            ...
            {isFormOpen && (
                <SearchSourceAddForm
                    ...
                    onSubmit={async (input) => {
                        const ok = await onAddSearchSource(input);
                        if (ok) setIsFormOpen(false);
                        return ok;
                    }}
                />
            )}
            <SearchSourceList sources={sources} title="Configured source details" />
        </section>
    );
}

function SearchSourceAddForm({
    isAdding,
    onCancel,
    onSubmit,
}: {
    ...
    const [draft, setDraft] = React.useState<SearchSourceDraft>({
        name: "",
    ...
    return (
        <form onSubmit={submit} className="mt-4 rounded-lg border border-cortex-border bg-cortex-bg/60 p-3">
            <div className="grid gap-3 md:grid-cols-2">
                <SourceField label="Source name">
                    <input
                        ...
                        className="w-full rounded-lg border border-cortex-border bg-cortex-surface px-3 py-2 text-xs text-cortex-text-main outline-none focus:border-cortex-primary/50"
                        placeholder="Company knowledge search"
                    />
                </SourceField>
                ...
                <SourceField label="Boundary">
                    <input
                        ...
                        onChange={(event) => setDraft((current) => ({ ...current, boundary: event.target.value }))}
                        className="w-full rounded-lg border border-cortex-border bg-cortex-surface px-3 py-2 text-xs text-cortex-text-main outline-none focus:border-cortex-primary/50"
                        placeholder="Approved company knowledge index"
                    />
                </SourceField>
                <SourceField label="Visible to">
                    <select
                        value={draft.scope_kind}
                        onChange={(event) => setDraft((current) => ({ ...current, scope_kind: event.target.value, scope_ref: event.target.value === "all" ? "" : current.scope_ref }))}
                        className="w-full rounded-lg border border-cortex-border bg-cortex-surface px-3 py-2 text-xs text-cortex-text-main outline-none focus:border-cortex-primary/50"
                    >
                ...
                {draft.scope_kind !== "all" && (
                    <SourceField label="Scope reference">
                        <input
                            ...
                            onChange={(event) => setDraft((current) => ({ ...current, scope_ref: event.target.value }))}
                            className="w-full rounded-lg border border-cortex-border bg-cortex-surface px-3 py-2 text-xs text-cortex-text-main outline-none focus:border-cortex-primary/50"
                            placeholder={draft.scope_kind === "group" ? "research-team" : "workstation-1"}
                        />
                    </SourceField>
                )}
                <SourceField label="Authentication">
                    <select
                        ...
                        onChange={(event) => setDraft((current) => ({ ...current, auth_scheme: event.target.value, secret_ref: event.target.value === "none" ? "" : current.secret_ref }))}
                        className="w-full rounded-lg border border-cortex-border bg-cortex-surface px-3 py-2 text-xs text-cortex-text-main outline-none focus:border-cortex-primary/50"
                    >
                ...
                {draft.auth_scheme !== "none" && (
                    <SourceField label="Secret reference">
                        <input
                            ...
                            onChange={(event) => setDraft((current) => ({ ...current, secret_ref: event.target.value }))}
                            className="w-full rounded-lg border border-cortex-border bg-cortex-surface px-3 py-2 text-xs text-cortex-text-main outline-none focus:border-cortex-primary/50"
                            placeholder="SEARCH_PROVIDER_API_KEY"
                        />
                        <span className="text-[10px] leading-4 text-cortex-text-muted">
                            Use a saved reference name, not the token value.
                        </span>
                    </SourceField>
                )}
            </div>
            ...
            <div className="mt-3 flex justify-end gap-2">
                <button type="button" onClick={onCancel} className="rounded-lg border border-cortex-border bg-cortex-surface px-3 py-2 text-xs font-semibold text-cortex-text-muted transition hover:text-cortex-text-main">
                    Cancel
                </button>
                <button type="submit" disabled={isAdding} className="rounded-lg border border-cortex-primary/30 bg-cortex-primary/10 px-3 py-2 text-xs font-semibold text-cortex-primary transition hover:bg-cortex-primary/20 disabled:cursor-not-allowed disabled:opacity-60">
                    {isAdding ? "Adding..." : "Add search source"}
                </button>
            </div>
        </form>
    );
...
function emptySourceLabel(addSupported: boolean): string {
...
function validateSearchSourceDraft(draft: SearchSourceDraft): string | null {
    ...
    if (requiresEndpoint(draft.source_type) && !draft.endpoint?.trim()) {
        return "Add the search endpoint Soma may use for this source.";
    }
    ...
    if (draft.auth_scheme !== "none") {
        ...
        if (secretRef.includes("=") || /\s/.test(secretRef) || /^sk-[A-Za-z0-9]/.test(secretRef)) {
            return "Use a saved secret reference name, not a raw secret value.";
        }
    }
    return null;
}
