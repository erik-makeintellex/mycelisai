...
export const GUIDED_ACTIONS: Array<{
...
}> = [
  {
    ...
    detail:
      "See the next priorities Soma would set so the workspace starts moving in a visible direction.",
  },
  {
    action: "focus_first",
    label: "Choose the first priority",
    detail:
      "Ask Soma to identify the first move most likely to create visible progress across the workspace.",
  },
  {
    action: "review_setup",
    label: "Review your organization setup",
    detail:
      "Check whether Advisors, Departments, and Specialists are ready and see what Soma recommends inspecting next.",
  },
  {
    action: "resume_retained_package",
    label: "Resume retained package",
    detail:
      "Recover the next step from durable outputs after a reboot, reload, or interruption without rebuilding finished work.",
  },
];
...
export function normalizeGuidanceResponse(
  ...
  const requestLabel = rewriteGuidanceText(
    sanitizeGuidanceText(payload?.request_label, defaultRequestLabel(action)),
    somaName,
    teamLeadName,
  );
  const headline = rewriteGuidanceText(
    sanitizeGuidanceText(
      payload?.headline,
      `${somaName} guidance for ${organizationName}`,
    ),
    somaName,
    teamLeadName,
  );
  const summary = rewriteGuidanceText(
    sanitizeGuidanceText(
      payload?.summary,
  ...
  const prioritySteps = normalizeGuidanceList(
    payload?.priority_steps,
    defaultPrioritySteps(action, organizationName),
    somaName,
    teamLeadName,
  );
  ...
  const executionSummary =
    payload?.execution_summary && typeof payload.execution_summary === "object"
...
export function defaultRequestLabel(action: TeamLeadGuidedAction) {
  return (
    GUIDED_ACTIONS.find((item) => item.action === action)?.label ??
    "Work with Soma"
  );
}
...
export function resolvePromptAction(prompt: string): TeamLeadGuidedAction {
  ...
  if (
    /(resume|reboot|reload|retained package|retained output|continue from|pick back up)/i.test(
      normalized,
    )
  ) {
  ...
  if (
    /(review|check|audit|inspect|understand|setup|structure)/i.test(normalized)
  ) {
    return "review_setup";
  }
  if (
    /(first|priority|focus|start with|begin with|most important)/i.test(
      normalized,
    )
  ) {
    return "focus_first";
...
function normalizeGuidanceList(
  value: unknown,
  fallback: string[],
  somaName: string,
  teamLeadName: string,
...
function normalizeExecutionContract(
  ...
  const targetOutputs = Array.isArray(contract.target_outputs)
    ? contract.target_outputs
        .map((entry) =>
          rewriteGuidanceText(
            sanitizeExecutionContractText(entry, ""),
            somaName,
            teamLeadName,
          ),
        )
  ...
  const workstreams = Array.isArray(contract.workstreams)
    ? contract.workstreams
        .map((entry) =>
          normalizeExecutionWorkstream(entry, somaName, teamLeadName),
        )
  ...
  return {
    ...
    owner_label: rewriteGuidanceText(
      sanitizeExecutionContractText(contract.owner_label, "Execution path"),
      somaName,
      teamLeadName,
    ),
    summary: rewriteGuidanceText(
      sanitizeExecutionContractText(
        contract.summary,
        "Soma has an execution path ready.",
      ),
      somaName,
      teamLeadName,
    ),
    continuity_label: rewriteGuidanceText(
      sanitizeExecutionContractText(contract.continuity_label, ""),
      somaName,
      teamLeadName,
    ),
    continuity_summary: rewriteGuidanceText(
      sanitizeExecutionContractText(contract.continuity_summary, ""),
      somaName,
      teamLeadName,
    ),
    resume_checkpoint: rewriteGuidanceText(
      sanitizeExecutionContractText(contract.resume_checkpoint, ""),
      somaName,
      teamLeadName,
    ),
    team_name: sanitizeExecutionContractText(contract.team_name, ""),
    external_target: sanitizeExecutionContractText(
      contract.external_target,
      "",
    ),
    target_outputs: targetOutputs,
    workstreams,
    workflow_group: normalizeWorkflowGroupDraft(
      contract.workflow_group,
      somaName,
      teamLeadName,
    ),
    recommended_team_shape: sanitizeExecutionContractText(
      contract.recommended_team_shape,
      "",
    ),
    coordination_model: sanitizeExecutionContractText(
      contract.coordination_model,
      "",
    ),
    recommended_team_count:
      typeof contract.recommended_team_count === "number"
        ? contract.recommended_team_count
        : undefined,
    initial_member_count:
      typeof contract.initial_member_count === "number"
        ? contract.initial_member_count
        : undefined,
    recommended_team_member_limit:
      typeof contract.recommended_team_member_limit === "number"
        ? contract.recommended_team_member_limit
        : undefined,
    expansion_policy: rewriteGuidanceText(
      sanitizeExecutionContractText(contract.expansion_policy, ""),
      somaName,
      teamLeadName,
    ),
    temporary_addition_guidance: rewriteGuidanceText(
      sanitizeExecutionContractText(contract.temporary_addition_guidance, ""),
      somaName,
      teamLeadName,
    ),
  };
...
function normalizeWorkflowGroupDraft(
  ...
  return {
    ...
    name: rewriteGuidanceText(
      sanitizeExecutionContractText(draft.name, "Temporary workflow group"),
      somaName,
      teamLeadName,
    ),
    goal_statement: rewriteGuidanceText(
      sanitizeExecutionContractText(
        draft.goal_statement,
        "Coordinate a focused workflow.",
      ),
      somaName,
      teamLeadName,
    ),
    work_mode: workMode,
    coordinator_profile: rewriteGuidanceText(
      sanitizeExecutionContractText(draft.coordinator_profile, "Workflow lead"),
      somaName,
      teamLeadName,
    ),
    ...
    initial_member_count:
      typeof draft.initial_member_count === "number"
        ? draft.initial_member_count
        : undefined,
    recommended_member_limit:
      typeof draft.recommended_member_limit === "number"
        ? draft.recommended_member_limit
        : undefined,
    expansion_policy: rewriteGuidanceText(
      sanitizeExecutionContractText(draft.expansion_policy, ""),
      somaName,
      teamLeadName,
    ),
    temporary_addition_guidance: rewriteGuidanceText(
      sanitizeExecutionContractText(draft.temporary_addition_guidance, ""),
      somaName,
      teamLeadName,
    ),
    expiry_hours:
      typeof draft.expiry_hours === "number" ? draft.expiry_hours : undefined,
    summary: rewriteGuidanceText(
      sanitizeExecutionContractText(
        draft.summary,
        "Launch a temporary workflow group from this Soma plan.",
      ),
      somaName,
      teamLeadName,
    ),
...
function normalizeExecutionWorkstream(
  ...
  const label = rewriteGuidanceText(
    sanitizeExecutionContractText(workstream.label, ""),
    somaName,
    teamLeadName,
  );
  const ownerLabel = rewriteGuidanceText(
    sanitizeExecutionContractText(workstream.owner_label, ""),
    somaName,
    teamLeadName,
  );
  const summary = rewriteGuidanceText(
    sanitizeExecutionContractText(workstream.summary, ""),
    somaName,
    teamLeadName,
  );
  const nextStep = rewriteGuidanceText(
    sanitizeExecutionContractText(workstream.next_step, ""),
    somaName,
    teamLeadName,
  );
...
export function rewriteGuidanceText(
  value: string,
  somaName: string,
  teamLeadName: string,
) {
