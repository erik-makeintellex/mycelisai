...
import { ChevronDown, ExternalLink, RotateCcw, ShieldCheck, Sparkles } from "lucide-react";
    ...
    <div className="rounded-lg border border-cortex-info/20 bg-cortex-info/5 px-3 py-2.5 shadow-sm" data-testid="execution-summary-card">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {executionStatus ? (
            <span className="rounded border border-cortex-success/20 bg-cortex-success/10 px-1.5 py-0.5 font-mono text-[9px] font-bold uppercase text-cortex-success">
              {executionStatus}
            </span>
          ) : null}
        </div>
      ...
      <div className="space-y-2">
        ...
        {!outputDigest && executionSummary ? <p className="text-xs leading-5 text-cortex-text-main">{executionSummary}</p> : null}
        {allOutputs.length > 0 ? <ExecutionSummaryMediaPreview outputs={allOutputs} compact /> : null}
        <div className="rounded border border-cortex-border/60 bg-cortex-surface/60 px-2 py-1.5 text-[11px] leading-5 text-cortex-text-muted">
        ...
        {degradation.length > 0 ? (
          <div className="rounded border border-red-400/30 bg-red-400/10 px-2 py-1.5 text-[11px] leading-5 text-red-200">
