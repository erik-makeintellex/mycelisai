...
export default function GroupManagementPanel({ initialSelectedGroupId = null }: { initialSelectedGroupId?: string | null }) {
    ...
    const buckets = useMemo<GroupBucket[]>(() => [
        { id: "standing", title: "Standing groups", groups: groups.filter((group) => group.status !== "archived" && !group.expiry) },
        { id: "temporary", title: "Temporary groups", groups: groups.filter((group) => group.status !== "archived" && !!group.expiry) },
        { id: "archived", title: "Archived temporary groups", groups: groups.filter((group) => group.status === "archived" && !!group.expiry) },
    ], [groups]);
    ...
    const loadGroups = async () => {
        ...
        try {
            ...
            setSelectedGroupId((current) => pickSelectedGroupId(nextGroups, current, initialSelectedGroupId));
    ...
    const broadcastToGroup = async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`/api/v1/groups/${encodeURIComponent(selectedGroup.group_id)}/broadcast`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
...
function pickSelectedGroupId(groups: Group[], current: string | null, requested: string | null) {
    if (requested && groups.some((group) => group.group_id === requested)) return requested;
    if (current && groups.some((group) => group.group_id === current)) return current;
