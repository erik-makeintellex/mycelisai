...
import TeamLeadInteractionPanel, {
  type TeamLeadPromptSuggestion,
} from "@/components/organizations/TeamLeadInteractionPanel";
import { extractApiData, extractApiError } from "@/lib/apiContracts";
import {
  readLastOrganization,
  subscribeLastOrganizationChange,
  type LastOrganizationRef,
} from "@/lib/lastOrganization";
...
const PROMPT_SUGGESTIONS: TeamLeadPromptSuggestion[] = [
  ...
  },
  {
    ...
    prompt:
      "Create a research team that can analyze customer interviews, summarize patterns, and produce an executive-ready recommendation brief.",
  },
  {
    label: "RevOps workflow team",
    prompt:
      "Create an operations team that can design a lead intake workflow, define the automation contract, and return the implementation checklist.",
  },
  {
    label: "Security review team",
    prompt:
      "Create a security review team that can assess a planned release, identify risks, and deliver an approval-ready review summary.",
  },
];

const WORKFLOW_HIGHLIGHTS = [
  "Start from the organization context Soma already knows instead of building a team from scratch.",
  "Ask for the team outcome, expected outputs, and delivery style before adding member details.",
  "Start with one accountable lead; if the ask is broad, let Soma split it into a few lead-only teams and add temporary specialists only when a lead names the gap.",
];

export default function TeamCreationPage() {
  const searchParams = useSearchParams();
  const [lastOrganization, setLastOrganization] =
    useState<LastOrganizationRef | null>(null);
  const [organization, setOrganization] =
    useState<OrganizationHomePayload | null>(null);
  ...
  const effectiveOrganizationId =
    searchParams?.get("organization_id") || lastOrganization?.id || null;
    ...
    const loadOrganization = async () => {
      ...
      try {
        const response = await fetch(
          `/api/v1/organizations/${effectiveOrganizationId}/home`,
          { cache: "no-store" },
        );
        ...
        if (!response.ok) {
          throw new Error(
            extractApiError(payload) ||
              "Unable to load the current AI Organization.",
          );
        }
        if (cancelled) return;
            ...
            <div className="max-w-3xl space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-2xl text-base leading-7 text-cortex-text-muted">
                  Use a guided workflow when you want to create a new team or
                  delivery lane. Soma will shape the team around the
                  organization context, expected outputs, and the kind of work
                  the team needs to deliver.
                </p>
              </div>
            ...
            <div className="rounded-2xl border border-cortex-border bg-cortex-bg px-4 py-4 text-sm text-cortex-text-muted lg:max-w-md">
              ...
              <div className="mt-4 rounded-2xl border border-cortex-border bg-cortex-surface px-4 py-3">
                ...
                <p className="mt-2 leading-6 text-cortex-text-muted">
                  Start with the team lead only. When the request spans
                  marketing, product, ops, or media in the same breath, Soma
                  should create a few lead-only lanes and add a temporary
                  specialist only after the lead names the missing capability,
                  owned task, expected proof, and removal point.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <section className="space-y-4 rounded-3xl border border-cortex-border bg-cortex-surface p-6">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-cortex-text-muted">
                Team creation is anchored to an AI Organization so Soma can use
                the right memory, priorities, and team defaults.
              </p>
            </div>
            ...
            {loadState === "error" && error ? (
              <div className="rounded-2xl border border-cortex-danger/30 bg-cortex-danger/10 px-4 py-4">
                <p className="text-sm font-semibold text-cortex-text-main">
                  The current AI Organization could not be loaded
                </p>
            ...
            {loadState === "ready" && !organization ? (
              <div className="rounded-2xl border border-dashed border-cortex-border bg-cortex-bg px-4 py-4">
                <p className="text-sm font-semibold text-cortex-text-main">
                  Choose or create an AI Organization first
                </p>
                <p className="mt-2 text-sm leading-6 text-cortex-text-muted">
                  Open Soma from the dashboard, enter an organization workspace,
                  and then come back here to create a team with the right
                  organization context already attached.
                </p>
                <div className="mt-4 flex flex-wrap gap-2">
                  <Link
                    href="/dashboard"
              ...
              <div className="rounded-2xl border border-cortex-border bg-cortex-bg p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-2 text-lg font-semibold text-cortex-text-main">
                      {organization.name}
                    </p>
                    <p className="mt-1 text-sm leading-6 text-cortex-text-muted">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  <ContextMetric
                    label="Team lead"
                    value={organization.team_lead_label}
                  />
                  <ContextMetric
                    label="Departments"
                    value={String(organization.department_count)}
                  />
                  <ContextMetric
                    label="Specialists"
                    value={String(organization.specialist_count)}
                  />
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  <Link
            ...
            <div className="rounded-2xl border border-cortex-border bg-cortex-bg p-4">
              ...
              <h3 className="mt-2 text-base font-semibold text-cortex-text-main">
                Tell Soma what success looks like
              </h3>
              <div className="mt-3 space-y-2 text-sm leading-6 text-cortex-text-muted">
                ...
                <p>
                  Name the deliverables the team should return, such as review
                  notes, campaign assets, workflow contracts, or approval-ready
                  recommendations.
                </p>
                <p>
                  Use the prompt chips in the creation panel if you want a quick
                  enterprise-oriented starting point, then let Soma keep the
                  default team lead-only unless the work proves a temporary
          ...
          <section className="space-y-4 rounded-3xl border border-cortex-border bg-cortex-surface p-6">
            <div>
              ...
              <h2 className="mt-2 text-xl font-semibold text-cortex-text-main">
                Run guided team design with Soma
              </h2>
              <p className="mt-1 text-sm leading-6 text-cortex-text-muted">
                This is the focused workflow for creating a team. Soma can plan
                the lane, decide whether the work should become a native team or
                external workflow contract, and return the outputs it expects
                the team to produce.
              </p>
            </div>
            ...
            {organization ? (
              ...
              <div className="rounded-2xl border border-dashed border-cortex-border bg-cortex-bg px-4 py-6">
                <p className="text-sm font-semibold text-cortex-text-main">
                  The guided creation lane appears once an AI Organization is
                  available
                </p>
                <p className="mt-2 text-sm leading-6 text-cortex-text-muted">
                  After you return to an organization workspace, this page will
                  open directly into Soma&apos;s team-design flow.
                </p>
              </div>
            ) : null}
