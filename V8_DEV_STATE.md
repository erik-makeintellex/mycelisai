...
- `COMPLETE` the latest clean WSL release proof pass from the refreshed `mother-brain` checkout is green on commit `a6b2aaf392e693149227ed49aca6d4a143dac5d5`: `uv run inv wsl.refresh` synced WSL to `origin/main`, then `uv run inv wsl.validate --lane=release` passed. Evidence included `uv run inv install`, `uv run inv ci.release-preflight --lane=runtime --no-e2e`, `uv run inv auth.posture --compose`, `uv run inv compose.up --build --wait-timeout=240`, `uv run inv compose.health`, `uv run inv compose.storage-health`, Windows-side `http://localhost:3000` reachability, and live Chromium `soma-governance-live.spec.ts`, `team-creation.spec.ts`, `groups-live-backend.spec.ts`, and `workspace-live-backend.spec.ts`. Compose health reported text inference online and media offline as a non-blocking note.
...
- `COMPLETE` release-candidate proof for the current pushed `origin/main` is green in the authoritative WSL/Compose lane as of 2026-04-30; the next release action is packaging/tag/handoff, not another local-only proof cycle, unless a new code change lands.
...
- `IN_REVIEW` the Soma web-capability contract now points to the governed Mycelis Search capability instead of claiming direct web access is impossible: `web_search` is the first path for search intent, `local_sources` and self-hosted `searxng` do not require Brave tokens, `fetch` remains the explicit URL retrieval path, and `brave-search` remains an optional MCP path when `BRAVE_API_KEY` is present.
