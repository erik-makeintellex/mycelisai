...
2. `ACTIVE` this lane is explicitly downstream of the universal-Soma correction and upstream of true-MVP closeout: approval moments now need to feel like trust interactions with one persistent Soma operating across governed contexts, not like isolated technical workflows bound to a single organization surface.
...
9. `NEXT` continue Slice 2 of the ask-class / agent-type / output-contract assertion lane.
   ...
   - decide whether specialist-shaped answers also need stronger default-visible outcome framing in addition to the existing badge plus consultation trace
