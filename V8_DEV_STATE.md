...
9. `IN_REVIEW` `/teams/create` start-mode safety is fixed, but the combined create -> open group -> review outputs headed-browser certification still needs one more stabilization slice against the current managed browser harness.
10. `NEXT` finish the enterprise workflow certification by reconnecting the create -> open group -> review outputs browser story through the guided `/teams/create` lane now that the route itself is production-build safe again.
