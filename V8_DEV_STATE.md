...
- `IN_REVIEW` the Soma web-capability contract now points to governed MCP capability instead of claiming direct web access is impossible: `fetch` remains the explicit URL retrieval path, `brave-search` is the configured web-search path when `BRAVE_API_KEY` is present, and Soma/Council web-capable roles must return a concrete Connected Tools blocker when the required MCP server or credential is missing.
