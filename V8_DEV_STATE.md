...
- `ACTIVE` compact-team defaults are now part of the product story: ordinary team creation should stay small and focused, while broad asks should be split into several smaller coordinated teams or lanes orchestrated by Soma with Council help over NATS and managed exchange instead of growing one monolithic roster.
...
- `NEXT` compact-team orchestration should be validated end to end across backend guidance, UI wording, managed exchange visibility, and browser proof so the default team size story stays small unless the request is explicitly broad.
