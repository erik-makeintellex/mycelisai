...
- `IN_REVIEW` the temporary group lifecycle lane now includes explicit archive with retained output review and a first Soma-mediated launch path: Mycelis supports manual temporary-group creation, lead handoff links, group broadcast while active, archived temporary-group separation, and retained output review after closure, and guided team creation can now launch a bounded temporary workflow group directly from the returned Soma execution path instead of dropping the operator back into raw group fields.
...
8. `NEXT` deepen the temporary workflow lane beyond creation: attach richer lead navigation and multi-output proof to Soma-launched groups so the enterprise path covers create -> coordinate -> produce outputs -> archive -> retained review in one browser-certified workflow.
