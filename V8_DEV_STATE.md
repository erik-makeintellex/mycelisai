...
- `ACTIVE` the next organization-entry browser follow-through is now narrowed to the recent-organization continuity path: the first four headed Chromium entry/create scenarios are green on the live UI, while the remaining reopen/return-to-organization flow still needs the setup-panel reopen path and remembered-organization path kept explicitly aligned.
...
- `ACTIVE` the temporary group lifecycle lane is now explicitly partial instead of implied-complete: Mycelis supports manual temporary-group creation, lead handoff links, group broadcast, and recent team-output review, but it still needs an explicit retire/archive path that preserves output review after the lane is closed, plus a future Soma-mediated creation path for the full enterprise workflow story.
...
4. `ACTIVE` the current browser-first organization-entry lane is now narrowed to continuity/re-entry rather than initial creation: the first four headed Chromium scenarios are green on the live UI, and the remaining work is to keep the recent-organization reopen/return contract aligned between the setup-panel list, remembered-organization affordance, and the live dashboard flow before widening back out to the rest of the serial spec.
5. `ACTIVE` temporary groups are now explicitly treated as a partial business workflow: create/broadcast/output review exist, but retirement with retained outputs and Soma-driven creation are still the next meaningful lifecycle slices for enterprise and mid-tier execution stories.
...
55. `COMPLETE` People & Access now has an explicit layered-permissions and product-layering contract instead of always exposing raw user CRUD: `/api/v1/user/me` and `/api/v1/user/settings` now carry `access_management_tier` plus the investor-review posture fields `product_edition`, `identity_mode`, and `shared_agent_specificity_owner`; `/api/v1/user/me` also now exposes normalized principal metadata (`principal_type`, `auth_source`, `effective_role`, `break_glass`) so the current local-admin and future break-glass recovery posture are inspectable in one contract; the settings UI keeps organization roles and collaboration groups in the base-release layer, enterprise user-directory management is exposed only when that layer is enabled, the deployment-access model makes self-hosted release vs self-hosted enterprise vs hosted-control-plane posture visible, backend `admin` identity now maps truthfully to owner access in the interface, and the workflow docs/tests now prove base-release, enterprise-owner, non-owner, and shared-Soma-output-ownership expectations. Proof is green with `cd core; go test ./internal/server -run 'TestHandle(Me|UserSettings|AuthMiddleware)' -count=1`, `cd interface; npx vitest run __tests__/settings/UsersPage.test.tsx __tests__/pages/SettingsPage.test.tsx --reporter=dot`, `cd interface; npx tsc --noEmit`, `uv run inv interface.e2e --project=chromium --spec=e2e/specs/settings.spec.ts`, and `$env:PYTHONPATH='.'; uv run pytest tests/test_docs_links.py -q`.
