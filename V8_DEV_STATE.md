...
- `IN_REVIEW` MCP library/catalog standardization is now in flight for the investor and operator lanes: the curated install flow is being preserved, while the Mycelis MCP library schema, config, and UI are being upgraded toward the standard `server.json` concepts used by the official MCP registry (`name`, `version`, package/transport definitions, repository metadata, and typed environment-variable declarations) so future agentry registration expands through a recognizable ecosystem contract instead of a Mycelis-only schema.
...
3. `IN_REVIEW` the first MCP standardization slice is now implemented locally and green in focused proof: the curated library schema now carries version/package metadata and typed environment-variable declarations, the Connected Tools browser renders that standardized metadata, and the operator docs/API reference now describe the catalog as a registry-aligned surface rather than a purely local install list.
...
5. `NEXT` continue MCP standardization with deeper interoperability follow-through: evaluate optional repository/homepage metadata, package-version pinning policy, and future import/export compatibility with published MCP `server.json` records before widening the registration surface.
