...
- `ACTIVE` a new structured team-ask lane is now in flight: `docs/architecture-library/V8_STRUCTURED_TEAM_ASKS_AND_LANE_ROUTING_PLAN.md` defines the canonical `TeamAsk` contract, lane-role model, backward-compatible `delegate_task` normalization path, and phased follow-through from command-bus preservation into agent/runtime specialization and team-instantiation defaults.
