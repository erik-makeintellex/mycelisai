...
- `IN_REVIEW` the `/teams/create` route is now production-build safe again: the route wraps the client guided-creation surface in a suspense boundary so start-mode/production builds no longer fail on `useSearchParams()`, while the combined headed browser proof for create -> open group still needs follow-through against the current managed browser harness.
- `IN_REVIEW` the managed Interface harness now also treats stale Windows `.next/standalone` cleanup locks as a retryable build/start artifact instead of a hard failure: `uv run inv interface.build` and start-mode browser runs clean repo-local Interface workers, reset the local build surface, retry once, and keep the next browser-first target focused on the real create -> open group -> review outputs workflow rather than host cleanup residue.
...
9. `IN_REVIEW` `/teams/create` start-mode safety is fixed, and the managed browser harness now retries once after stale `.next/standalone` cleanup locks, but the combined create -> open group -> review outputs headed-browser certification still needs one more stabilization slice against the current managed browser harness.
10. `NEXT` finish the enterprise workflow certification by reconnecting the create -> open group -> review outputs browser story through the guided `/teams/create` lane now that the route itself is production-build safe again and the highest-value Windows cleanup residue path is covered.
