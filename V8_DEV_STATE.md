...
5. `NEXT` Phase 0 truth mapping is the immediate implementation gate: classify every approval field into default-visible, details-only, or runtime-only, then classify request types into inline answer, governed artifact, optional approval, and required approval before editing the proposal-card surface.
6. `NEXT` the first UI actuation slice should simplify the default proposal card so it answers what Soma wants to do, why approval is needed, what will change, and what the user can decide, while moving internal tool/runtime/expression detail behind an explicit details control.
7. `NEXT` after proposal-surface simplification, the team must close the content/artifact value-delivery loop so drafting requests return inline value and durable outputs return explicit artifact references or previews instead of leaving operators to infer that work happened.
8. `NEXT` the compose live fresh-organization direct-answer defect remains part of this lane’s release risk until proven fixed or clearly reclassified; approval/product-trust work cannot be considered truly clean while a healthy compose stack can still surface a first-path `500` in the live browser gate.
