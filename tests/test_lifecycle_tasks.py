...
def test_status_uses_core_healthz_when_tcp_snapshot_misses(monkeypatch, capsys):
    ...
    monkeypatch.setattr(lifecycle, "_port_open", lambda *args, **kwargs: False)
    monkeypatch.setattr(lifecycle, "_find_pid_on_port", lambda _port: None)
    monkeypatch.setattr(lifecycle, "_list_compiled_go_service_processes", lambda: [])

    def fake_http_get(url: str, timeout: float = 3.0, headers=None):
    ...
    output = capsys.readouterr().out
    assert "Core API        : UP" in output
    assert "Core API probe  : HEALTHY" in output
    assert "Core API probe  : OFFLINE" not in output


def test_status_uses_ollama_tags_when_tcp_snapshot_misses(monkeypatch, capsys):
    class Result:
        ok = True
        exited = 0
        stdout = ""
    ...
    output = capsys.readouterr().out
    assert "Ollama          : UP" in output
    assert "Ollama probe    : HEALTHY" in output
