...
- `ACTIVE` P0.10 keeps Mycelis central execution as the safe default while building the framework-neutral `framework_runs` Runs API boundary. Core now mints the only run id; persists its exact proof/contract/work/source/revision binding, event receipts, contiguous cursor, approvals, and control commands; and can atomically project accepted evidence. External-create intent is committed as `awaiting_handler`, which the current dispatcher cannot claim, so no worker request or backend selection is enabled. External completion remains an unverified candidate. Delivery order is preserve the central authority/finalization reference path -> neutral durability/control gate -> existing LangGraph driver certification -> optional CrewAI compatibility/import compiler -> later AG2 evaluation. The bundled Python service remains deterministic conformance tooling; the exact source, ownership, deployment, proof, and handoff plan is the canonical PRD's [P0.10 Framework Execution Delivery Referential](docs/architecture-library/MYCELIS_CANONICAL_PRD.md#p010-framework-execution-delivery-referential).
