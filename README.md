...
Go owns runtime, orchestration, API, NATS, and backend persistence-facing logic. TypeScript owns the interface and in-app docs browser. Python owns repo management, operator automation, CI orchestration, and local test harnesses. SQL owns migrations, and `db.migrate` compatibility gates include the current capability, proof, trust, team-work lifecycle, collaboration-group workspace-folder, OutcomeProject, TeamRegistry, search-source registry, dispatch-outbox, operator SSE event-ledger, and team signal-receipt schema. `uv run inv db.clear-runtime-context` is the guarded source-mode reset for stale Soma/team runtime context before fresh UX proof. PowerShell may only be a thin host wrapper when the platform requires it.
