...
- `IN_REVIEW` P0.10 keeps Mycelis central execution as the default and sole confirmed-Outcome backend while adding a framework-neutral `framework_runs` Runs API boundary. The bundled Python service is deterministic conformance tooling only; external selection fails closed until durable correlated event projection exists, and every external completion remains an unverified candidate for Core validation. LangGraph is the first optional dependency-gated driver; AG2 and CrewAI remain future adapter/import paths, and AutoGen is not added.
- `IN_REVIEW` P0.7d adds LiteLLM as an optional model-gateway target behind the existing OpenAI-compatible provider contract. Core remains the provider-policy and profile-routing authority; LiteLLM may normalize provider transport, credentials, operational rate/spend ceilings, and approved-boundary retries, but it cannot select a forbidden data boundary, approve work, orchestrate teams, or establish Outcome proof. The Python SDK is not embedded in Core, the proxy is disabled by default, and no LiteLLM service or production persistence is installed by this slice.
