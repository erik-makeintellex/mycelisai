...
- `IN_REVIEW` P0.10 now has Core-owned durable authority plus the separate Go `services/framework-runs` Runs API and PostgreSQL journal. The service implements the seven authenticated neutral routes, exact replay/conflict semantics, crash-recoverable commands and leases, schema verification, and immutable candidate manifests, but intentionally injects no production executor and reports `production_ready=false`; Core still cannot dispatch or select it. External completion remains an unverified candidate. Delivery order is preserve the central authority/finalization reference path -> neutral durability/control gate -> disabled deployment package -> post-commit dispatch/reconciliation -> Core finalization -> LangGraph certification -> optional CrewAI compatibility/import compiler -> later AG2 evaluation. The bundled Python facade remains deterministic conformance tooling; the exact source, ownership, deployment, proof, and handoff plan is the canonical PRD's [P0.10 Framework Execution Delivery Referential](docs/architecture-library/MYCELIS_CANONICAL_PRD.md#p010-framework-execution-delivery-referential).
...
- The authenticated Dashboard is the primary Soma Workspace. Interface and API owners retain the existing organization route identifiers and `AI Organization` return label through the V8.3 compatibility lifetime so saved links and retained work continue to open; remove them only after a Workspace-native route migration supplies redirects, API migration, updated navigation copy, retained-state proof, and browser regression coverage. They must not introduce a competing product hierarchy.
...
Cleanup note: `uv run inv clean.generated` removes allowlisted repo-local dependencies, build output, reports, tool caches, and source-tree `__pycache__` directories. It targets only `core/workspace/tool-cache` and `interface/workspace/tool-cache`, not either whole workspace, and leaves `.env`, runtime logs, Compose data, Docker volumes, and the active Python runtime untouched. If you intentionally need to remove `.venv`, do it from an external shell after leaving the environment; use `uv run inv clean.disk-status` to inspect the same target set first.
...
- an obsolescence review removes replaced code, aliases, configuration, tests, fixtures, docs, and generated artifacts or records an explicit compatibility owner, lifetime, and removal gate
- deployment config copies match their canonical sources byte-for-byte; deployment differences belong in documented values or environment overrides
