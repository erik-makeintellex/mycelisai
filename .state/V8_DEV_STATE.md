...
| Active slice | External-mutation recovery is integrated on `dev`. An uncertain external result now requires one durable operator verification (`committed`, `not_committed`, or `still_unknown`) before any follow-up; no branch automatically retries the mutation. Focused protocol/API persistence and delayed-signal projection tests, Core build, 16 UI tests, typecheck, docs links, line policy, fresh service health, and the 7-journey headed Teams suite pass on both the feature and merged integration states. Manual novice acceptance is the remaining product gate. |
...
1. Complete manual novice acceptance for the integrated P0.9 deterministic package journey and the verification-first recovery language.
2. Freeze the candidate and run P0.12 exact-candidate preflight, Compose/Kubernetes deployment proof, and operator browser certification.
3. Promote certified `dev` to `main`, rerun release smoke and health, then synchronize remotes intentionally.
4. Resume Worker Profile editing and optional provider-specific media coverage only after the candidate is accepted.
