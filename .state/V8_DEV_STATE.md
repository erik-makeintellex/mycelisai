...
| Active slice | P0.7b-D2a scoped Worker Profile activation and new-team lineage is `IN_REVIEW` for integration on `dev`. Activated ConfigDocuments are the sole custom-profile runtime authority; trusted confirmation boundaries own scope, injected lineage fails closed, and the primary UI hands creation/customization to Soma instead of writing legacy rows. D2b durable manifest restoration is `NEXT`. P0.9 remains `IN_REVIEW` only for manual novice acceptance. |
...
| P0.7b declarative configuration authoring | `ACTIVE` | D1 is integrated with strict Worker Profile ConfigDocument validation and immutable preview snapshots. D2a resolves custom profiles directly from the activation index in operator, workspace, organization, then built-in order; only the trusted confirmed request boundary supplies scope. New team members pin exact custom record/version/digest/tenant/scope lineage, forged snapshots and unresolved refs fail closed, explicit approved member fields remain authoritative, and fallback is limited to locked `source=built_in` rows. Resources and Teams expose ready-made profiles as inspect-only and move creation/customization into a prepared Soma conversation rather than certifying obsolete catalogue CRUD. Full Core and Interface suites, lint, typecheck, production build, docs links, max-lines, all-route restart, and five headed Chromium profile journeys pass. | Persist and restore the full hydrated runtime manifest across Core restart, then prove activation B affects only new teams, rollback returns later teams to A, owned cleanup, and the live Soma create/activate/use/revisit journey. |
...
2. Finish P0.7b-D2b durable Worker Profile manifest restoration and the live activation/rollback journey; keep authoring Soma-first until the scope-aware active-profile read model exists.
