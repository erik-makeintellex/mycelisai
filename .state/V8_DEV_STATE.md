...
| Active slice | P0.9 human-first Soma convergence is `ACTIVE`. Retained deliverables now open in a dedicated Mycelis output canvas, conversational completion links use that surface, and Back restores the originating Soma query, Outcome/team context, and anchor. |
...
| Main release risk | Asynchronous steering during genuinely delayed work and unprompted novice comprehension still require explicit acceptance. External-mutation recovery acceptance and exact-candidate deployment certification remain release gates; optional provider-specific media proof is not the critical path. |
...
| --- | --- | --- | --- |
| P0.9 full Trusted Outcome Journey | `ACTIVE` | Governed ask through revisit remains proven. The human-first slice keeps the untouched composer visible, prevents automatic drawers/runtime vocabulary/horizontal overflow, preserves a compact Work drawer, and collapses repeated machine updates into one user-relevant state. Retained documents, applications, media, and packages open in a dedicated Mycelis canvas; completion links enter that canvas instead of the raw file endpoint, and one Back action restores the originating Soma query, Outcome/team context, and anchor. Focused component plus headed desktop/compact interaction, overflow, and error proof pass. | Prove conversational steering during genuinely delayed work and run manual novice comprehension before release certification. |
...
2. Run manual novice acceptance for Ask, active work, and completed deliverable discovery without instruction.
3. Review external-mutation recovery, then execute P0.12 certification and intentional `dev -> main` promotion only if every release gate passes.
