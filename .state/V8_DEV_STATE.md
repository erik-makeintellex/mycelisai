...
| Working tree at checkpoint | Runtime/UI live proof update: `/dashboard?fresh=1` is now covered by a focused business-owner browser contract that rejects stale retained output/media/chat content before the ask, then requires proposal approval to create a retained output. The stored-plan handoff for `write_file` approval replay is fixed and browser proof created `E:\random\test\generated\business-owner-flow-1780423995673\owner-note.md`. |
...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard work contexts now appear only when there is focused or active/output work, so standing teams no longer crowd the clean root Soma surface. Ask Team now appears as queued non-blocking work immediately, Active Work polls active durable rows until terminal state, correlated team signals can advance queued rows to output-ready/degraded, retained `outputs[]`/`output_refs[]` project back onto the original row, and focused team contexts now show compact retained output access before the side panel is opened. Proposal approval now shows immediate running feedback and result visibility. Output summaries make latest file/folder access the first visible action and move secondary output proof behind details. `/dashboard?fresh=1` clears browser-persisted Soma chat/session scopes for fresh interaction testing. Live business-owner proof confirms clean fresh dashboard entry, approval, retained output, and generated workspace file on visible port `3000`. | Continue full browser review of richer team/media workflows, then keep simplifying trust/proof detail into collapsed review affordances by default. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
