...
| Field | State |
| --- | --- |
| Current committed checkpoint | `Prioritize generated team outputs` |
| Working tree at checkpoint | Team engagement slice: dashboard team context wording now reads as `All Soma work` / `Team focus`, active/running/degraded team work opens the Work review panel first and suppresses duplicate output previews, and live team-output proof now verifies durable API readback for `TeamWorkItem`, `TeamStatusEvent`, workspace-path `output_refs`, reveal response semantics, generated-file opening, dashboard re-entry, and Groups retention. |
