...
| Docs/State | `ACTIVE` | API reference, automation user docs, and this scoreboard reflect the committed scheduler handoff slice, next-phase kickoff, live GUI proof, live ComfyUI media proof, and current max-line convergence proof. | Keep docs/state synchronized in the same slice as behavior, API, runtime, workflow, testing, or terminology changes. | docs/workflow tests, `git diff --check`, max-lines. |
