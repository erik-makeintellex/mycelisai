...
| Active slice | P0.7b Worker Profile lineage is `IN_REVIEW` on its feature branch. Live headed Soma proof now saves/activates A, creates team A, saves/activates B, creates team B without mutating A, rolls back to A, creates team A2, reloads exact manifests, and purges owned state. Team creation commits through the dispatch outbox before profile hydration so approval returns immediately; bounded profile/persistence contexts prevent a blocked backend from freezing team reads. Post-merge `dev` proof remains before acceptance. P0.9 remains `IN_REVIEW` only for manual novice acceptance. |
...
2. Merge the proven Worker Profile lineage slice to `dev` and repeat the headed A/B/rollback/A2/reload acceptance there.
