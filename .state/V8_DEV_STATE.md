...
| Active slice | P0.7b-D1 Worker Profile configuration foundation is integrated and `COMPLETE` on `dev`. The next bounded slice is scoped activation projection and runtime lineage; a user-facing editor remains gated until one authority and owned cleanup are proven. P0.9 remains `IN_REVIEW` only for manual novice acceptance. |
...
| P0.7b declarative configuration authoring | `ACTIVE` | Worker Profiles, team manifests, Soma command manifests, MCP connections, Search Sources, registered inputs, and NATS buffering already have bounded runtime contracts. D1 is integrated on `dev` with strict Worker Profile validation, kind-dispatched compiler/API/tool preview, family-correct Soma preview language, and immutable id/version/digest snapshots. Feature and post-merge proof cover the full Core and Interface suites, lint, typecheck, focused confirmation-state tests, compile, docs links, max-lines quality, service health, and an authenticated live YAML dry-run; the live query confirms preview retained zero revisions. The locked built-in profile catalogue remains compatible, and custom profile activation must not create a second authority. | Add scoped activation projection, exact runtime lineage, owned QA cleanup, and the focused Soma/Resources authoring journey as the next bounded slice. |
...
2. Implement P0.7b-D2 scoped Worker Profile activation and runtime lineage before exposing profile authoring in Resources.
