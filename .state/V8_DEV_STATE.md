...
| Active slice | The P0.7b Worker Profile runtime-lineage sub-slice is `COMPLETE` on `dev`. Worker Profiles are now explicitly governed as inert catalogue templates: save, preview, activation, rollback, list, and read have no agent, provider/API, session, or NATS startup side effects. Approved `create_team` is the sole instantiation boundary and resolves only selected profiles before starting member runtime resources. The rebuilt merged binary previously passed live headed Soma A/B/rollback/A2 lineage proof; the inert-catalogue regression, focused Core suites, full Go build, docs links, line policy, and interface typecheck pass. The focused Resources editor/direct-file authoring journey is `NEXT`; P0.9 remains `IN_REVIEW` only for manual novice acceptance. |
