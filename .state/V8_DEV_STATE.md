...
| Active slice | P0.7b Worker Profile lineage is `IN_REVIEW` on `feature/p0-7b-config-command-resilience` after the primary lineage slice merged to `dev`. Live headed Soma proof saves/activates A, creates team A, saves/activates B, creates team B without mutating A, rolls back to A, creates team A2, re-fetches exact manifests after browser navigation, and purges owned state; focused persistence tests separately prove Core restart restoration. Inline save, exact activate, and versioned rollback commands compile without model inference, while incomplete or decision-shaped asks stay conversational. Post-merge `dev` proof remains for this resilience follow-up. P0.9 remains `IN_REVIEW` only for manual novice acceptance. |
...
| Main release risk | Deterministic package generation and live opening now pass. Manual novice comprehension, external-mutation recovery acceptance, integrated `dev` proof, and exact-candidate deployment certification remain release gates; optional provider-specific media proof is not the critical path. |
...
| P0.7b declarative configuration authoring | `IN_REVIEW` | D1 is integrated with strict Worker Profile ConfigDocument validation and immutable preview snapshots. D2a resolves custom profiles directly from the activation index in operator, workspace, organization, then built-in order; only the trusted confirmed request boundary supplies scope. D2b adds an authoritative PostgreSQL runtime-team manifest with schema version and canonical digest, exact full-manifest Core restart restoration, idempotent identical save, conflicting replacement rejection, and durable stop/mission deactivation. Direct HTTP callers cannot inject profile refs or snapshots; only Soma's governed resolver establishes lineage. Team creation now dispatches only after the confirmation transaction commits, avoiding custom-profile lookup self-blocking; profile resolution and durable persistence also honor bounded caller contexts without holding the team read lock. Focused Core tests and the headed live A/B/rollback/A2/navigation re-fetch/cleanup journey pass on the feature branch. Resources and Teams keep ready-made profiles inspect-only and creation/customization Soma-first. | Merge the resilience follow-up to `dev`, rerun Core, type, docs, health, and headed lineage proof, then build the focused Resources editor/direct-file acceptance slice. |
...
1. Complete manual novice acceptance for the integrated P0.9 deterministic package journey.
2. Merge the Worker Profile command-resilience follow-up to `dev` and repeat the headed A/B/rollback/A2/navigation re-fetch acceptance there.
3. Review external-mutation recovery and execute P0.12 certification only if every release gate passes.
4. Exercise optional providers, including direct Forge media delivery, as capability-specific coverage without displacing the core release journey.
