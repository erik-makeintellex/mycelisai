...
| Field | State |
| --- | --- |
| Current committed checkpoint | `4cc4d7fd Use Soma result receipts` |
| Working tree at checkpoint | Active enterprise workflow UI cleanup continuation: Soma simple-mode chat now uses compact execution receipts for trusted results, preserving the latest-output open/folder action and small media preview while keeping full output/proof/detail review in the Work panel, Runs, Activity, or Resources surfaces. |
...
93. `npx tsc --noEmit` and `npm test -- MissionControlChat.executionSummary SomaWorkspaceFrame OutputWorkbench` pass after simple-mode Soma result cards were compressed to latest-output actions plus small media preview and visible degraded-run boundaries. Full output lists, proof details, capability rows, and verbose recovery remain behind Work panel/run/resource detail surfaces.
94. `npm test -- MissionControlChat.executionSummary MissionControlChat.executionSummaryMedia SomaWorkspaceFrame` passes after trusted simple-mode execution summaries became concise receipts with direct latest-output actions, media preview, and full proof/detail preserved outside the main chat lane.
95. `npx tsc --noEmit`, `uv run pytest tests/test_docs_links.py tests/test_workflow_contracts.py tests/test_runtime_deploy_contract_text.py -q`, `uv run inv quality.max-lines --limit 300`, and `uv run inv interface.e2e --server-mode=start --project=chromium --workers=1 --spec=e2e/specs/desktop-mobile-compression.spec.ts` pass after the result-receipt slice.
96. `npm test -- MissionControlChat.header SomaOperatingSurface.actions` and `npx tsc --noEmit` pass after Root Soma/team context copy was made explicit in the switcher, focus bar, and team-scoped chat header.
97. `npm test -- MissionControlChat.header SomaOperatingSurface.actions useCortexStore.mission-chat-send` and `npx tsc --noEmit` pass after team-scoped chat sends were made workspace-scope authoritative over stale selected-team state.
98. `uv run inv interface.e2e --server-mode=start --project=chromium --workers=1 --spec=e2e/specs/desktop-mobile-compression.spec.ts` and `uv run inv interface.e2e --server-mode=start --project=chromium --workers=1 --spec=e2e/specs/focused-team-output-dashboard.spec.ts` pass after the Root Soma/focused-team copy and current verification disclosure labels were aligned with browser proof.
99. `npm test -- CentralSomaHome useCortexStore.mission-chat-send MissionControlChat.header SomaOperatingSurface.actions` and `npx tsc --noEmit` pass after dashboard organization id propagation, root re-entry stale-team clearing, unknown `team_id` fallback, and removal of dead Soma surface message derivation.
100. `uv run inv interface.e2e --server-mode=start --project=chromium --workers=1 --spec=e2e/specs/desktop-mobile-compression.spec.ts` and `uv run inv interface.e2e --server-mode=start --project=chromium --workers=1 --spec=e2e/specs/focused-team-output-dashboard.spec.ts` pass after the dashboard scope/re-entry cleanup, confirming the compressed dashboard and focused-team output lane still load from a managed production build.
101. `uv run inv interface.e2e --server-mode=start --project=chromium --workers=1` passes with 87 tests passed and 46 explicitly skipped after current Soma-first browser copy, package output controls, Settings setup guidance, Groups broadcast summary, and legacy organization-entry skip posture were aligned with the production Interface bundle.
