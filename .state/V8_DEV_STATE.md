...
| Working tree at checkpoint | Active approval/output/fresh-dashboard UX correction: proposal confirmation now gives immediate pending/executing feedback, the store marks approved proposals as awaiting proof while backend execution runs, team-plus-media confirmations no longer claim that no work item started, the output workbench exposes a primary Latest output card before audit details, focused team chat context includes latest retained output refs, `/dashboard?fresh=1` purges browser-persisted Soma chat/session scopes, and the clean root dashboard no longer surfaces standing teams as active work contexts. |
...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard work contexts now appear only when there is focused or active/output work, so standing teams no longer crowd the clean root Soma surface. Ask Team now appears as queued non-blocking work immediately, Active Work polls active durable rows until terminal state, correlated team signals can advance queued rows to output-ready/degraded, retained `outputs[]`/`output_refs[]` project back onto the original row, and focused team contexts now show compact retained output access before the side panel is opened. Proposal approval now shows immediate executing feedback and pending-proof state. Output summaries make latest file/folder access the first visible action and move secondary output proof behind details. `/dashboard?fresh=1` clears browser-persisted Soma chat/session scopes for fresh interaction testing. Live login/dashboard, dashboard workbench scroll containment, governed execution, Ask Team, live team-generated output, focused output dock, retained-media, and focused ComfyUI journey proof are green. | Continue full browser review of approval-to-output interaction clarity, then simplify the trust package into collapsed/overlay detail by default. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
