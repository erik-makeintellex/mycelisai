...
| Active slice | Release-candidate remediation is isolated on `feature/release-candidate-preflight`. The first clean baseline exposed one stale recovery assertion and a Windows `.next` lock retained by a managed Playwright listener. Recovery proof now requires verification before retry, and the managed Interface stop path clears browser listeners before production builds. The four browser expectations caught by the next preflight now certify current product behavior: Help navigation, conversational `start`, deliberate advanced-team disclosure, and in-Mycelis output review. Focused Core, task lifecycle, docs-link, line-policy, production-build, dashboard, team-workspace, focused-output, and Trusted Outcome Journey gates pass; the clean full preflight remains the promotion boundary. |
