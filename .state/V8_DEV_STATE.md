...
| Working tree at checkpoint | Team-output usability/live-proof slice: the latest output digest and full workbench now prefer generated file-like artifacts over team/group folders, so the primary `Open file` action opens the deliverable a user asked Soma to create. The live team-output browser proof now creates a team, writes a playable HTML output, opens the generated file, verifies the game interaction, reveals the local folder, returns to the focused dashboard, and verifies Groups retention. |
