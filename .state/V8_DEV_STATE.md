...
| Working tree at checkpoint | Browser proof maintenance slice: current Soma copy uses `run`, compact result/recovery cards use `Needs review`, `Still available`, and `Not reliable`, package/media output proofs now assert the current output-first controls, and legacy AI Organization setup specs are identified as stale against the Soma-first dashboard entry. |
...
| Main release risk | Async team ask acceptance, correlated status/result projection, retained output-ref projection, focused output access, and live team output review are operator-visible. Remaining release-candidate risk is retiring stale legacy organization-entry specs, keeping current Soma dashboard proof green, and running live provider media proof only when ComfyUI/gateway flags are intentionally enabled. |
