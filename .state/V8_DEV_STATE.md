...
| Working tree at checkpoint | Active async team execution slice: Team Ask now consumes the async accepted work-item response, shows an immediate queued/degraded/operator notice, merges submitted rows into Soma and Teams Active Work, and polls queued/running/reviewing durable rows until terminal state. Backend async acceptance already exists; worker-result correlation/output-ref hardening remains next. |
...
| Main release risk | Async team ask acceptance is now operator-visible, but full release-candidate proof still needs correlated team worker results, persisted output refs from async result signals, and richer team-specific output views so generated content and active work stay obvious without topology interpretation |
...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard now exposes a `Work contexts` strip that switches root Soma and team-focused lanes while chat, active work, retained outputs, and proof move together. Ask Team now appears as queued non-blocking work immediately, and Active Work polls active durable rows until terminal state. Live login/dashboard proof is green after aligning the homepage spec to the current `Soma environment` label. Dashboard workbench proof shows contained scrolling and retained-output/folder affordances on native services. Governance, package reload, media output, focused ComfyUI journey, and recovery/degradation are proven across focused specs. | Continue from context switching and async acceptance into correlated async team outputs and richer team-specific output views. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
...
| P0 | Async team execution | Ask Team now queues without blocking the operator and keeps Active Work polling while work is active. Next, preserve `work_item_id` through worker result signals and persist output refs/artifacts from correlated async results. | Runtime Teams + QA/Embodiment | Team work starts quickly, remains visible in the focused context, and returns retained output/proof or actionable degradation. |
