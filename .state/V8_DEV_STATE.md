...
| Field | State |
| --- | --- |
| Current committed checkpoint | `Expand focused team browser proof` |
| Working tree at checkpoint | Runtime/UI live proof update: `/dashboard?fresh=1` is covered by a headed business-owner browser contract that rejects stale retained output/media/chat content before the ask, shows approval/running feedback, then creates a retained output with obvious file and folder actions. The stored-plan handoff for `write_file` approval replay is fixed and latest browser proof created `E:\random\test\generated\business-owner-flow-1780436488993\owner-note.md`. |
...
| Focused browser proof | Full Chromium UI pass on 2026-06-02 covered login/front door, dashboard workbench scroll containment, focused team output dock, live governed execution, live Ask Team, live team-generated playable output, Groups retention, retained media, and focused ComfyUI media journey. Recovery/degradation UI proof passes in mocked mode. The headed business-owner live dashboard proof now passes on visible port `3000`: clean fresh entry, proposal approval/running feedback, retained output, obvious file/folder actions, generated workspace file, output rail showing the current file instead of `Guided proposal`, and review-panel scroll containment without page scroll. |
| Product posture | Governed schedule proposals are durable and reviewable; scheduler ticks do not execute work autonomously |
| Main release risk | Async team ask acceptance, correlated status/result projection, retained output-ref projection, focused output access, and live team output review are operator-visible. Remaining release-candidate risk is browser-certified interaction clarity across richer team/media workflows, active execution, output discovery, and follow-up context so operators do not have to infer whether work started or where the latest artifact lives. |
...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard work contexts now appear only when there is focused or active/output work, so standing teams no longer crowd the clean root Soma surface. Ask Team now appears as queued non-blocking work immediately, Active Work polls active durable rows until terminal state, correlated team signals can advance queued rows to output-ready/degraded, retained `outputs[]`/`output_refs[]` project back onto the original row, and focused team contexts now show compact retained output access before the side panel is opened. Proposal approval now shows immediate running feedback and result visibility. Output summaries now present a simple `Result` card with the latest output first, larger `Open file` and `Open folder` actions, proof-backed/openable outputs only, and secondary request/proof/recovery details collapsed by default. `/dashboard?fresh=1` clears browser-persisted Soma chat/session scopes for fresh interaction testing. Headed live browser proof confirms clean fresh dashboard entry, approval/running feedback, retained output, generated workspace file, review rail current-file selection, and contained review-panel scrolling on visible port `3000`. | Continue full browser review of richer team/media workflows, then carry the same result-first pattern into team/media/package outputs and reduce remaining dashboard setup text. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
...
80. `$env:PLAYWRIGHT_LIVE_BACKEND='1'; $env:PLAYWRIGHT_SKIP_WEBSERVER='1'; $env:PLAYWRIGHT_PORT='3000'; npx playwright test e2e/specs/dashboard-workbench-live-review.spec.ts --project=chromium --headed` passes in a visible browser: clean business-owner entry, approval/running feedback, retained output, `Open file`, `Open folder`, generated workspace file `E:\random\test\generated\business-owner-flow-1780436488993\owner-note.md`, review rail showing the current file instead of `Guided proposal`, and contained side-rail scrolling with page scroll remaining at `0`.
81. `npm test -- --run __tests__/dashboard/SomaCausalSummary.test.tsx` and `npx tsc --noEmit` pass after extracting `SomaCausalSummaryModel.ts`; `SomaCausalSummary.tsx` is no longer a max-line violation. `uv run inv quality.max-lines --limit 300` still fails on remaining known files: `cognitive_chat_helpers_test.go`, `cognitive_tool_plan_parse.go`, `confirm_action_visibility_test.go`, and `ProposedActionBlock.tsx`.
...
| P0 | Team output usability | Focused team contexts now surface retained output refs in a compact Team outputs dock while preserving the detailed output list in the Work panel/Team page. Live team output proof now creates a playable file, opens it, reveals the folder, returns to focused Dashboard, and verifies the Team outputs dock. | Media Lane + Visual/UI | Generated media appears in the relevant team/output context with one-click file/folder access and proof/recovery nearby. |
...
| P1 | File-size convergence | Continue isolated max-line cleanup on remaining known violations after the proven UI/runtime slice is committed. | Quality | `quality.max-lines` has no uncapped violations and behavior-focused tests remain green. |
