...
| Field | State |
| --- | --- |
| Current committed checkpoint | `Surface Soma output digest in workbench` |
| Working tree at checkpoint | Runtime/UI live proof update: `/dashboard?fresh=1` is covered by a headed business-owner browser contract that rejects stale retained output/media/chat content before the ask, shows approval/running feedback, then creates a retained output with obvious file and folder actions in the output digest. The stored-plan handoff for `write_file` approval replay is fixed, latest browser proof created `E:\random\test\generated\business-owner-flow-1780453021517\owner-note.md`, remaining uncapped backend max-line files are split into focused helper files, and focused-team Soma context now carries selected team IDs into proposal wiring/conversation persistence while newest focused-team outputs outrank older root Soma outputs. |
...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard work contexts now appear only when there is focused or active/output work, so standing teams no longer crowd the clean root Soma surface. Ask Team now appears as queued non-blocking work immediately, Active Work polls active durable rows until terminal state, correlated team signals can advance queued rows to output-ready/degraded, retained `outputs[]`/`output_refs[]` project back onto the original row, and focused team contexts now show compact retained output access before the side panel is opened. Proposal approval now shows immediate running feedback and result visibility. Focused team chats now preserve selected team context through backend conversation logging/proposal wiring, and the frontend prefers newest focused-team outputs before older root Soma outputs. Output summaries present a simple `Result` card with the latest output first, larger `Open file` and `Open folder` actions, visible workspace path hints, proof-backed/openable outputs only, and secondary request/proof/recovery details collapsed by default. Folder reveal feedback now uses direct user language (`Folder opened` / `Open failed`). `/dashboard?fresh=1` clears browser-persisted Soma chat/session scopes for fresh interaction testing. Headed live browser proof confirms clean fresh dashboard entry, approval/running feedback, retained output, output digest file/folder access, generated workspace file, review rail current-file selection, Output-first review opening, and contained review-panel scrolling on visible port `3000`. | Run live browser proof of a focused team/media/package journey after this context/output-ref slice, then continue reducing dashboard setup text. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
| API/Runtime Contract | `IN_REVIEW` | Trigger execution history includes schedule handoff proof fields, and `POST /api/v1/triggers/{id}/history/{executionId}/approval` transitions persisted schedule handoff state only. Focused `/api/v1/chat` requests preserve selected `team_id` for proposal wiring and conversation turns, while durable team `output_refs[].storage_ref` now stores workspace paths/folders instead of viewer URLs. | Browser-certify the combined focused-team output journey and keep API docs synchronized for the next runtime contract change. | Go API tests, docs/API reference review, history reload proof, focused live GUI proof. |
