...
| Delivery target | Trusted Outcome Journey and exact release-candidate certification are complete, and local `dev -> main` promotion is complete. Current development is tightening complex generated-output delivery so Soma can create a profile-backed temporary team, hand work to the bus, return a retained output, and truthfully expose validation failure as repair-needed work when output is not usable. |
...
1. Finish the code-generated media/team handoff slice: keep the media fallback, keep canonical team bus input normalization, document the live proof result, and avoid claiming any unsatisfied generated candidate as delivered.
2. Build the next bounded-output closure slice: long-running teams must emit timely progress/degradation, `reviewing` validation must have a short recovery SLA, and complex app packages must fail visibly when validation proves a blank or unusable output.
