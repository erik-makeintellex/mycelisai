...
1. Promote certified `dev` to `main`, then rerun source health and headed browser smoke.
2. Synchronize `main` and `dev`, remove the merged feature branch, and verify local/remote cleanliness.
3. Resume Worker Profile editing, broader deployment certification, and optional provider-specific media coverage only after this candidate is promoted.
