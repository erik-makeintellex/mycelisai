...
| Active slice | Code-generated media/team handoff proof is active on `feature/code-generated-media-fallback`. Soma now distinguishes explicit image/media generation from browser-native code-generated visuals, Worker Profile team metadata preserves the canonical NATS command lane, failed result-contract candidates no longer appear as delivered output, and interactive packages must provide retained package files plus a validated state-changing browser control before trusted completion. |
...
| Delivery target | Trusted Outcome Journey and exact release-candidate certification are complete, and local `dev -> main` promotion is complete. Current development has tightened complex generated-output delivery so Soma can create a profile-backed temporary team, hand work to the bus, return a retained project package, validate the browser interaction, and truthfully expose validation failure as repair-needed work when output is not usable. |
| Main release risk | Complex generated app/game delivery now has passing headed proof for the generic package lane, but quality remains model-dependent. Release still needs manual novice review of the completion language, direct launch path, repair guidance, and explicit external-media provider proof when the media generator is intentionally available. |
...
| Profile-backed complex app delivery | `IN_REVIEW` | Headed live proof created, saved, and activated a workspace Worker Profile; then Soma proposed and approved temporary app/game team delivery through chat. The team restores and subscribes to `swarm.team.{team_id}.internal.command` even when profile inputs include human-readable fields. Result contracts now require `index.html`, support files, package folder/entrypoint metadata, exact entrypoint readback, and contract-selected browser validation before trusted completion. Runtime-owned package scaffolding records support-file evidence instead of leaving generated packages half-trusted. Generic click workflows now validate text/state change while explicit held-key/movement workflows remain visual-change probes. Failed result-contract candidates keep artifacts for recovery but do not render as usable delivered output. The headed natural Soma delivery proof and headed Trusted Outcome Journey both pass against the live app stack after these changes. | Run manual novice review of the completion/output card, then keep this proof in release certification. |
...
1. Run manual novice review on the latest generated-output completion path: concise done message, direct open action, proof/recovery depth behind Details, and obvious follow-up through Soma.
2. Keep the generic complex-app live GUI proof in the release certification set: Soma ask through approval, async team work, retained `project_package`, UI open, interaction state change, proof/revisit, and cleanup.
