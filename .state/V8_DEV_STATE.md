...
| Active slice | P0.3a bounded discovery and Outcome Templates is integrated on `dev` and remains `IN_REVIEW`. P0.9 deterministic package delivery remains `ACTIVE`; the next gate is proving that Soma can use the resolved brief conversationally without adding interface machinery. |
...
| P0.3a bounded discovery and Outcome Templates | `IN_REVIEW` | A shared `mycelis.ai/v1` ConfigDocument contract now provides strict YAML/JSON parsing, path-root guards, structured validation, canonical digest, immutable PostgreSQL revisions, kind-scoped activation history, explicit rollback, and dry-run. Outcome Templates compile a bounded minimum brief into WorkIntent with version/digest snapshot and template/operator/policy precedence. Authenticated live API proof created, compiled, activated, superseded, rolled back, and removed test revisions; migration `061` upgraded the retained Docker PostgreSQL volume. Soma exposes preview/store/activate tools through governed command manifests. Full Core, 52 schema/docs tests, size gate, typecheck, browser docs/runs proof, and service health pass again after merge to `dev`. | Complete natural Soma browser proof for draft -> preview -> store -> activate -> use, then either accept P0.3a or correct only the observed conversational gap. |
...
1. Finish P0.3a conversational acceptance: have Soma draft, preview, store, activate, and apply one Outcome Template while the operator remains in the thread.
2. Make package-contract correction deterministic using that resolved brief so workers finish required writes, entrypoint readback, and validation within the bounded loop.
