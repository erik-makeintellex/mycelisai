...
| Active slice | P0.7b Resources Worker Profile authoring is integrated on `dev`. Inline dry-run preview uses the real ConfigDocument preview proxy, and save/activation continue through governed Soma handoff without starting agents, providers, sessions, or bus subscriptions. |
...
1. Clean the merged Worker Profile feature branch and verify `dev` stays clean.
2. Run any desired human review on Resources -> Worker Profiles, especially preview wording and Soma handoff clarity.
3. Continue the next P0 slice on a fresh feature branch after deciding whether this `dev` checkpoint should promote toward `main`.
