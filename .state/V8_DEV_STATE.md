...
| Active slice | Blocker configuration-link guidance is active on `feature/blocker-config-links`. Soma setup and unreachable blockers need to explain the exact problem, link the relevant Mycelis page, and say when the operator can ask Soma to prepare the configuration change through governed approval. |
...
1. Finish and merge the blocker configuration-link guidance slice, including focused chat failure tests, Markdown link tests, typecheck, docs-link proof, and clean branch state.
