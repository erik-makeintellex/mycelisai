...
| Field | State |
| --- | --- |
| Current committed checkpoint | `Persist async team output refs` |
| Working tree at checkpoint | Active focused-team output usability slice: when a Dashboard team context is selected and retained output refs exist, Soma now shows a compact Team outputs dock with open/open-folder/proof affordances before the operator opens the Work panel. The detailed output list remains in the existing workbench panel and Team page to avoid squeezing Soma's chat surface. |
...
| Main release risk | Async team ask acceptance, correlated status/result projection, retained output-ref projection, and focused output access are now operator-visible, but full release-candidate proof still needs a live end-to-end team output journey and an explicit artifact-ledger policy for inline team-generated artifacts |
...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard now exposes a `Work contexts` strip that switches root Soma and team-focused lanes while chat, active work, retained outputs, and proof move together. Ask Team now appears as queued non-blocking work immediately, Active Work polls active durable rows until terminal state, correlated team signals can advance queued rows to output-ready/degraded, retained `outputs[]`/`output_refs[]` project back onto the original row, and focused team contexts now show a compact Team outputs dock with open/open-folder/proof affordances before the side panel is opened. Live login/dashboard proof is green after aligning the homepage spec to the current `Soma environment` label. Dashboard workbench proof shows contained scrolling and retained-output/folder affordances on native services. Governance, package reload, media output, focused ComfyUI journey, and recovery/degradation are proven across focused specs. | Prove the focused output dock in a live end-to-end team output journey and decide artifact-ledger policy for inline team outputs. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
...
| P0 | Team output usability | Focused team contexts now surface retained output refs in a compact Team outputs dock while preserving the detailed output list in the Work panel/Team page. Next, prove a live team output journey that produces media/files and lands them in this focused context. | Media Lane + Visual/UI | Generated media appears in the relevant team/output context with one-click file/folder access and proof/recovery nearby. |
