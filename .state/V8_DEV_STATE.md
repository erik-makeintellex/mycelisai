...
| Field | State |
| --- | --- |
| Current committed checkpoint | `Make team asks visibly async` |
| Working tree at checkpoint | Active async team execution slice: Team Ask now consumes the async accepted work-item response, shows an immediate queued/degraded/operator notice, merges submitted rows into Soma and Teams Active Work, polls queued/running/reviewing durable rows until terminal state, and preserves `work_item_id` through team status/result signals so queued asks can project back to output-ready or degraded rows. Output-ref/artifact persistence from async result payloads remains next. |
...
| Main release risk | Async team ask acceptance and correlated status/result projection are now operator-visible, but full release-candidate proof still needs persisted output refs/artifacts from async result signals and richer team-specific output views so generated content and active work stay obvious without topology interpretation |
...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard now exposes a `Work contexts` strip that switches root Soma and team-focused lanes while chat, active work, retained outputs, and proof move together. Ask Team now appears as queued non-blocking work immediately, Active Work polls active durable rows until terminal state, and correlated team signals can advance queued rows to output-ready/degraded. Live login/dashboard proof is green after aligning the homepage spec to the current `Soma environment` label. Dashboard workbench proof shows contained scrolling and retained-output/folder affordances on native services. Governance, package reload, media output, focused ComfyUI journey, and recovery/degradation are proven across focused specs. | Continue from correlated async team output state into output-ref/artifact persistence and richer team-specific output views. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
...
| P0 | Async team execution | Ask Team now queues without blocking the operator, keeps Active Work polling while work is active, and preserves `work_item_id` through team status/result signals. Next, persist output refs/artifacts from correlated async results. | Runtime Teams + QA/Embodiment | Team work starts quickly, remains visible in the focused context, and returns retained output/proof or actionable degradation. |
