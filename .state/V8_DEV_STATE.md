...
| Delivery target | Trusted Outcome Journey and exact release-candidate certification are complete, and local `dev -> main` promotion is complete. Catalogue expansion, provider-specific polish, and new configuration families remain outside this candidate. |
| Main release risk | Deterministic package generation and live opening now pass. Exact-candidate release and post-promotion browser certification gates are complete locally; remaining close-out risk is branch synchronization/push and keeping new feature work out of this candidate. Optional provider-specific media proof and the deferred Worker Profile editor are not part of this candidate. |
