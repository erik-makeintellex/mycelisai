...
| Active slice | Workspace blocker guidance cleanup is active on `feature/workspace-unreachable-chat-guidance`. Soma setup and unreachable blockers need to explain the exact problem and next recovery step directly in the chat thread instead of only pointing to an operational alert. |
...
| Delivery target | Trusted Outcome Journey and exact release-candidate certification are complete, and local `dev -> main` promotion is complete. Current development is tightening setup and outage blockers so Soma remains conversational and actionable when workspace chat, media, or other optional providers are unavailable. |
| Main release risk | Deterministic package generation and live opening now pass. Exact-candidate release and post-promotion browser certification gates are complete locally; current risk is ensuring setup and outage blockers do not look like failed execution or leave users without a clear next step. |
...
| Media Outcome execution | `ACTIVE` | Direct Forge `/sdapi/v1/txt2img`, group-scoped media targeting, provider-specific recovery, broad Core tests, UI unit tests, typecheck/build, and headed Chromium preflight pass. With the running Forge UI returning `404` for `/sdapi/v1/options`, Soma now creates no proposal, approval, team, failed run, or internal planning deliverable. Current UI cleanup makes the Soma thread itself show setup and unreachable blocker reasons plus recovery steps instead of a vague operational-alert redirect. | Finish focused UI/typecheck/docs proof, merge the slice to `dev`, then restart Forge with API mode (`--api`) enabled and complete the headed natural Soma ask through generated image preview, saved group path, proof, and revisit. |
...
1. Finish and merge the workspace blocker guidance slice, including focused chat failure tests, typecheck, docs-link proof, and clean branch state.
2. When Forge is reachable with API mode enabled, run the natural Soma media ask through generated image preview, retained group media path, proof, and revisit.
3. Continue the next P0 slice on a fresh feature branch after confirming optional-provider blockers remain concise, actionable, and non-executing.
