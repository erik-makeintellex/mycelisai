...
| Active slice | P0.7b-D1 Worker Profile configuration foundation is `ACTIVE` on `feature/p0-7b-worker-profile-config`. The slice extends the proven ConfigDocument envelope with strict Worker Profile validation and kind-dispatched preview/compile before scoped activation projection or a user-facing editor is allowed. P0.9 remains `IN_REVIEW` only for manual novice acceptance. |
...
| P0.7b declarative configuration authoring | `ACTIVE` | Worker Profiles, team manifests, Soma command manifests, MCP connections, Search Sources, registered inputs, and NATS buffering already have bounded runtime contracts. D1 adds strict Worker Profile validation, kind-dispatched compiler/API/tool preview, family-correct Soma preview language, and immutable id/version/digest snapshots. Focused tests, the full Core suite, compile, docs links, max-lines quality, restarted service health, and authenticated live dry-run pass; the live query confirms preview retained zero revisions. The locked built-in profile catalogue remains compatible, and custom profile activation must not create a second authority. | Add scoped activation projection, exact runtime lineage, owned QA cleanup, and the focused Soma/Resources authoring journey as the next bounded slice. |
...
2. Complete P0.7b-D1 Worker Profile validation and compilation, then implement scoped activation/runtime resolution before exposing profile authoring in Resources.
