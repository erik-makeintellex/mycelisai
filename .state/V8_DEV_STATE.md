...
| Active slice | `feature/p0-11-docs-convergence` |
...
| P0.11 documentation convergence | `IN_REVIEW` | The canonical PRD contains the worker execution contract and all product architecture. The redundant architecture index/source-map documents are removed, the state transcript contains current truth only, and focused docs/Core/Interface/headed Help proof passes. | Commit, merge to `dev`, rerun affected proof, and remove the feature branch. |
...
| P0.12 release hygiene and certification | `NEXT` | Feature-to-dev merge discipline, retained browser reports, release-preflight ordering, and PostgreSQL/NATS runtime boundaries are implemented. | Run clean committed release preflight, WSL validation, Compose/Kubernetes proof, and browser certification. |
...
1. Complete P0.11 documentation convergence and remove the merged feature branch after post-merge proof.
...
5. Validate the guarded `uv run inv wsl.validate --lane=release` path from the refreshed WSL proof checkout, then certify Compose and Kubernetes runtime paths.
6. Run broader headed browser certification from committed state and retain JSON/JUnit evidence.
7. Promote `dev` to `main` only after every required gate passes.
