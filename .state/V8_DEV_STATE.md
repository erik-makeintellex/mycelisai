...
| Active slice | Media setup guidance cleanup is active on `feature/media-setup-chat-guidance`. Soma image-generation setup blockers now need to explain the exact provider problem and next recovery step directly in the chat thread instead of only pointing to an operational alert. |
...
| Delivery target | Trusted Outcome Journey and exact release-candidate certification are complete, and local `dev -> main` promotion is complete. Current development is tightening provider/setup blockers so Soma remains conversational and actionable when media or other optional providers are unavailable. |
| Main release risk | Deterministic package generation and live opening now pass. Exact-candidate release and post-promotion browser certification gates are complete locally; current risk is ensuring optional-provider blockers do not look like failed execution or leave users without a clear next step. |
...
1. Finish and merge the media setup guidance slice, including focused chat failure tests, typecheck, docs-link proof, and clean branch state.
