...
| Active slice | Shared NATS service boundaries are `COMPLETE` and merged into `dev`: explicit broker host selection, stable deployment client identity, concrete source-owned ingress channels, buffered external traffic, and post-merge service health are proven. P0.4 external-mutation uncertainty remains `IN_REVIEW`; operator review remains before release certification. |
