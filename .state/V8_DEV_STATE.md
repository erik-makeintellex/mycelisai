...
| Active slice | P0.3a bounded discovery and Outcome Templates is `IN_REVIEW` on its feature branch. P0.9 deterministic package delivery remains `ACTIVE`; the next gate is proving that Soma can use the resolved brief conversationally without adding interface machinery. |
...
| P0.3a bounded discovery and Outcome Templates | `IN_REVIEW` | A shared `mycelis.ai/v1` ConfigDocument contract now provides strict YAML/JSON parsing, path-root guards, structured validation, canonical digest, immutable PostgreSQL revisions, activation history, explicit rollback, and dry-run. Outcome Templates compile a bounded minimum brief into WorkIntent with version/digest snapshot and template/operator/policy precedence. Authenticated live API proof created, compiled, activated, superseded, rolled back, and removed test revisions; migration `061` was applied to the retained Docker PostgreSQL volume. Soma exposes preview/store/activate tools through governed command manifests. | Complete natural Soma browser proof for draft -> preview -> store -> activate -> use, then either accept P0.3a or correct only the observed conversational gap. |
