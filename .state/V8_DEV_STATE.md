...
| Active slice | P0.3a bounded discovery and Outcome Templates remains `IN_REVIEW`. Natural configured asks now route to preview/store/activate, direct-answer mutation calls fail closed, and authenticated live browser proof validates an exact template without persistence. P0.9 deterministic package delivery remains `ACTIVE`; the next P0.3a gate is retained store -> activate -> apply proof in the same Soma thread. |
...
| P0.3a bounded discovery and Outcome Templates | `IN_REVIEW` | A shared `mycelis.ai/v1` ConfigDocument contract provides strict YAML/JSON parsing, path-root guards, structured validation, canonical digest, immutable PostgreSQL revisions, kind-scoped activation history, explicit rollback, and dry-run. Outcome Templates compile a bounded minimum brief into WorkIntent with version/digest snapshot and template/operator/policy precedence. Config-backed Soma quotes now select preview/store/activate behavior. The direct-answer runtime hides and rejects mutation tools before proposal capture, while exact inline preview dispatches through the real validator without persistence. Focused Core, UI receipt, typecheck, mocked/headed save-activate-recovery, and authenticated live preview proof pass for this slice. | Complete one live Soma thread through retained store -> approve -> activate -> apply in governed work, verify reload and cleanup, then accept P0.3a or correct only the observed gap. |
...
1. Finish P0.3a retained conversational acceptance: use the live-proven preview in one Soma thread, then store, approve, activate, apply, reload, and clean up one Outcome Template.
