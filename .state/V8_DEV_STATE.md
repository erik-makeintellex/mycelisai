...
1. Complete the clean release-fix preflight and merge the bounded remediation into `dev`.
2. Rerun the exact-candidate gate on merged `dev`, promote certified `dev` to `main`, then rerun source health and headed browser smoke.
3. Synchronize `main` and `dev`, remove the merged feature branch, and verify local/remote cleanliness.
4. Resume Worker Profile editing, broader deployment certification, and optional provider-specific media coverage only after this candidate is promoted.
