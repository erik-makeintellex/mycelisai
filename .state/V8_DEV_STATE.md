...
| Active slice | Release-candidate remediation is isolated on `feature/release-candidate-preflight`. The first clean baseline exposed one stale recovery assertion and a Windows `.next` lock retained by a managed Playwright listener. Recovery proof now requires verification before retry, and Windows cleanup waits long enough for repo-owned process-tree termination instead of leaving child Next.js servers behind. The four browser expectations caught by the next preflight now certify current product behavior: Help navigation, conversational `start`, deliberate advanced-team disclosure, and in-Mycelis output review. Focused Core, task lifecycle, docs-link, line-policy, production-build, dashboard, team-workspace, focused-output, and Trusted Outcome Journey gates pass. A later focused rerun then exposed that the Windows process-inventory path could time out on a busy multi-project host and report a false clean before browser startup; the current working slice replaces that with one bounded repo-scoped Node/cmd query and has focused unit proof. Clean browser rerun and full preflight remain the promotion boundary. |
...
1. Finish the release-candidate cleanup commit: selector fix, bounded repo-scoped Windows process inventory, docs, and state.
2. Rerun the focused browser journey that exposed the issue, then complete the clean release-fix preflight and merge the bounded remediation into `dev`.
