...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard work contexts now appear only when there is focused or active/output work, so standing teams no longer crowd the clean root Soma surface. Ask Team now appears as queued non-blocking work immediately, Active Work polls active durable rows until terminal state, correlated team signals can advance queued rows to output-ready/degraded, retained `outputs[]`/`output_refs[]` project back onto the original row, and focused team contexts now show compact retained output access before the side panel is opened. Proposal approval now shows immediate running feedback and result visibility. Focused team chats now preserve selected team context through backend conversation logging/proposal wiring, and the frontend prefers newest focused-team outputs before older root Soma outputs. Output summaries present a simple `Result` card with the latest output first, larger `Open file` and `Open folder` actions, proof-backed/openable outputs only, and secondary request/proof/recovery details collapsed by default. `/dashboard?fresh=1` clears browser-persisted Soma chat/session scopes for fresh interaction testing. Headed live browser proof confirms clean fresh dashboard entry, approval/running feedback, retained output, output digest file/folder access, generated workspace file, review rail current-file selection, Output-first review opening, and contained review-panel scrolling on visible port `3000`. | Run live browser proof of a focused team/media/package journey after this context/output-ref slice, then continue reducing dashboard setup text. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
