...
| Active slice | P0.9 deterministic package delivery is `IN_REVIEW` on its feature branch. Natural Soma delivery now completes required files, exact entrypoint readback, browser interaction validation, retained proof, direct opening, steering, and scoped cleanup in one headed journey. |
...
| P0.9 full Trusted Outcome Journey | `IN_REVIEW` | Governed ask through revisit is proven. Contract correction resets only after successful evidence progress while retaining the hard 20-iteration bound. Project packages require current writes for required files and local dependencies, rendered validation markers, a usable interaction path across inline or referenced JavaScript, and a Core-owned exact readback of the latest entrypoint bytes before worker completion. Stale or divergent readback is rejected. The headed Chromium journey passes natural ask, approval, async NATS handoff, active-work steering, retained package generation, runtime browser validation, concise completion, safe embedded opening, visible Restart, observable Play behavior, proof, and fixture cleanup in 2.8 minutes. The dedicated output canvas also passes desktop and compact proof. | Merge and re-prove on `dev`, then run manual novice comprehension before release certification. |
...
1. Merge P0.9 deterministic package delivery into `dev`, rerun integrated Core and headed journey proof, and complete manual novice acceptance.
2. Extend the proven configuration path through P0.7b adapters, beginning with the smallest reusable profile/action family rather than widening every family at once.
...
| Generated package delivery and correlation | `IN_REVIEW` | Worker correction requests one missing contract item at a time and resets its correction allowance only after new successful evidence. Completion requires required-file and local-dependency writes, rendered validation markers, exact latest-entrypoint readback, and digest-bound browser interaction proof. The current headed feature proof completes steering, package delivery, embedded opening, and visible state change in 2.8 minutes; prior command-correlation proof remains accepted. | Merge and repeat the live journey on `dev`, then preserve it in release certification. |
