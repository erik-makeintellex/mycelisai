...
| Active slice | P0.9 deterministic package delivery is integrated on `dev` and remains `IN_REVIEW` only for manual novice acceptance. Natural Soma delivery completes required files, exact entrypoint readback, browser interaction validation, retained proof, direct opening, steering, and scoped cleanup in one headed journey. |
