...
| Active slice | Code-generated media/team handoff proof is active on `feature/code-generated-media-fallback`. Soma must distinguish explicit image/media generation from browser-native code-generated visuals, and Worker Profile team metadata must not break the canonical NATS command lane. |
...
1. Finish the code-generated media/team handoff slice: keep the media fallback, keep canonical team bus input normalization, document the live proof result, and avoid claiming the generated voxel game as delivered.
2. Build the next bounded-output closure slice: long-running teams must emit timely progress/degradation, and complex app packages must fail visibly when validation proves a blank or unusable output.
