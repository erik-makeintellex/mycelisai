...
| P0.12 release hygiene and certification | `ACTIVE` | Prior Windows release proof passes lint, Core, build, typecheck, Vitest, 121 Chromium journeys, source service health, and all four live governance scenarios. The current clean baseline then caught one stale recovery assertion and one retained Playwright build lock before promotion; both have bounded release-fix coverage. The release browser rerun exposed four obsolete expectations rather than product regressions; focused-output 1/1, dashboard 7/7, team workspace 7/7, and Trusted Outcome Journey 1/1 now pass against the current conversational and inspect-on-demand UX. The rebuilt production Interface and a start-mode browser cycle passed once, then a follow-up focused rerun exposed false-clean risk in Windows process inventory on a busy host. The current fix uses one bounded repo-scoped process inventory before killing only this checkout's Interface workers; focused unit proof, docs-link proof, typecheck, and focused first-demo Chromium browser proof pass. Review identified the next gap: `lifecycle.up`, `lifecycle.down`, `interface.stop`, and managed port cleanup still need explicit ownership checks so release proof cannot borrow or stop another checkout's services. Configured Compose ports, shared identity-secret boundaries, and the Docker PostgreSQL/NATS plus local Core/Interface development topology remain the accepted development posture. | Enforce service/port ownership, complete clean feature and merged-`dev` preflight, promote the exact candidate, then rerun source health and headed browser smoke on `main`. |
...
1. Enforce service/port ownership so release proof accepts this checkout's Core/Interface only and reports foreign listeners as conflicts.
2. Commit the visible rail Resources-access fix and ownership enforcement with focused tests.
3. Complete the clean release-fix preflight and merge the bounded remediation into `dev`.
4. Rerun the exact-candidate gate on merged `dev`, promote certified `dev` to `main`, then rerun source health and headed browser smoke.
5. Synchronize `main` and `dev`, remove the merged feature branch, and verify local/remote cleanliness.
6. Resume Worker Profile editing, broader deployment certification, and optional provider-specific media coverage only after this candidate is promoted.
