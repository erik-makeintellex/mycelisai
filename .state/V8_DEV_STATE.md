...
| Active slice | P0.12 container promotion on `feature/promotion-compose-port-certification`; configured host-port readiness and the Interface-to-Core forwarded-identity boundary are repaired and ready for `dev` integration. |
...
| P0.12 release hygiene and certification | `ACTIVE` | Clean Windows release preflight passes lint, Core, build, typecheck, Vitest, 121 Chromium journeys, native service health, and all four live governance scenarios. WSL Compose honors configured ports; after the shared identity-secret repair, Linux runtime preflight, auth posture, Compose health/storage, cognitive warmup, governance `4/4`, team creation, Groups retention, and workspace status all passed. The WSL-only final host probe encountered distro quiescence and is not the primary deployment lane. | Merge the certified feature into `dev`, rerun integration proof, then certify the supported local container/Kubernetes and Windows-browser path without keeping a full WSL proof stack resident. |
...
4. Certify the supported local Compose/Kubernetes runtime and Windows-browser path; use guarded WSL proof only when that environment supplies distinct evidence.
