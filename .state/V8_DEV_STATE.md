...
| Field | State |
| --- | --- |
| Current committed checkpoint | `ae845200 Surface confirm-action team work refs` |
| Working tree at checkpoint | Active enterprise workflow UI cleanup slice: Soma simple-mode chat now renders compact result cards with latest-output actions and small media previews while full proof/output detail remains in the Work panel or run/resource pages; docs now record the workflow-tool UI pattern checkpoint for Airflow/Temporal/GitHub Actions/Linear-style separation. |
