...
| Active slice | Shared NATS service boundaries are `IN_REVIEW`: explicit broker host selection, stable deployment client identity, concrete source-owned ingress channels, and buffered external traffic are implemented on a feature branch. P0.4 external-mutation uncertainty remains `IN_REVIEW`; operator review remains before release certification. |
...
| Shared NATS service boundary | `IN_REVIEW` | One Core process uses explicit `NATS_URL`; `MYCELIS_NATS_SERVICE_ID` names its runtime/observer clients on a shared broker. Registered external inputs own concrete `swarm.global.input.*` subjects, duplicate/wildcard claims fail closed, and existing source buffers protect teams from high-rate traffic. Routine app shutdown drains Mycelis clients and leaves shared broker state intact. | Complete live shared-broker registration/ingest proof, merge to `dev`, and rerun post-merge Core/service health. |
