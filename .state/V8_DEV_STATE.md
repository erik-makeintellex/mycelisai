...
| Active slice | Release-candidate remediation is isolated on `feature/release-candidate-preflight`. The first clean baseline exposed one stale recovery assertion and a Windows `.next` lock retained by a managed Playwright listener. Recovery proof now requires verification before retry, and Windows cleanup waits long enough for repo-owned process-tree termination instead of leaving child Next.js servers behind. The four browser expectations caught by the next preflight now certify current product behavior: Help navigation, conversational `start`, deliberate advanced-team disclosure, and in-Mycelis output review. Focused Core, task lifecycle, docs-link, line-policy, production-build, dashboard, team-workspace, focused-output, and Trusted Outcome Journey gates pass. A later focused rerun exposed that the Windows process-inventory path could time out on a busy multi-project host and report a false clean before browser startup; the current slice replaces that with one bounded repo-scoped Node/cmd query and has focused unit proof. The first rail-scoped output link proof then failed because the review rail hid Resources access under details; the rail now exposes `Open in Resources` beside `Open app`, and the focused first-demo Chromium proof passes again. Full preflight then caught duplicate accessible `Open in Resources` links when details were expanded and a parallel Core package race around local browser-output validation; the details link is now distinct, and release baseline Core tests run serially with `-p 1`. Service ownership is fail-closed: `lifecycle.up`, `lifecycle.down`, `interface.stop`, and managed listener cleanup accept only this checkout's Core/Interface processes and leave foreign listeners untouched with explicit operator messages. Full clean feature preflight remains the promotion boundary. |
| Production branch | `main` remains behind the accepted `dev` integration train pending release certification. |
| Runtime posture | Dockerized PostgreSQL/pgvector and NATS are running as the reusable data plane with Core and Interface running locally from source for feature proof; Ollama remains independently managed. Full Compose app containers, Kubernetes, and WSL remain explicit release-proof lanes. |
| Delivery target | Close the Trusted Outcome Journey, certify one exact release candidate, and intentionally promote `dev -> main`. Catalogue expansion, provider-specific polish, and new configuration families remain outside this candidate. |
| Main release risk | Deterministic package generation and live opening now pass. Exact-candidate release and post-promotion browser certification remain the immediate gates; optional provider-specific media proof and the deferred Worker Profile editor are not part of this candidate. |
...
| P0.12 release hygiene and certification | `ACTIVE` | Prior Windows release proof passes lint, Core, build, typecheck, Vitest, 121 Chromium journeys, source service health, and all four live governance scenarios. The current clean baseline then caught one stale recovery assertion and one retained Playwright build lock before promotion; both have bounded release-fix coverage. The release browser rerun exposed four obsolete expectations rather than product regressions; focused-output 1/1, dashboard 7/7, team workspace 7/7, and Trusted Outcome Journey 1/1 now pass against the current conversational and inspect-on-demand UX. The rebuilt production Interface and a start-mode browser cycle passed once, then a follow-up focused rerun exposed false-clean risk in Windows process inventory on a busy host. The current fix uses bounded repo-scoped process inventory before killing only this checkout's Interface workers; `lifecycle.up`, `lifecycle.down`, `interface.stop`, and managed listener cleanup now fail closed on foreign app listeners. Focused ownership tests, docs-link proof, line-policy proof, focused first-demo Chromium browser proof, OutputWorkbench accessible-link proof, docs/task-runner proof, and serial Core sweep pass. Release baseline Core tests now run with `-p 1` to protect local runtime/browser validation from package-level contention. Configured Compose ports, shared identity-secret boundaries, and the Docker PostgreSQL/NATS plus local Core/Interface development topology remain the accepted development posture. | Complete clean feature and merged-`dev` preflight, promote the exact candidate, then rerun source health and headed browser smoke on `main`. |
...
1. Commit the visible rail Resources-access fix and ownership enforcement with focused tests.
2. Complete the clean release-fix preflight and merge the bounded remediation into `dev`.
3. Rerun the exact-candidate gate on merged `dev`, promote certified `dev` to `main`, then rerun source health and headed browser smoke.
4. Synchronize `main` and `dev`, remove the merged feature branch, and verify local/remote cleanliness.
5. Resume Worker Profile editing, broader deployment certification, and optional provider-specific media coverage only after this candidate is promoted.
