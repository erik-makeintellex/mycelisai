...
| Active slice | Code-generated media/team handoff proof is active on `feature/code-generated-media-fallback`. Soma must distinguish explicit image/media generation from browser-native code-generated visuals, Worker Profile team metadata must not break the canonical NATS command lane, failed result-contract candidates must never be described as delivered output, and interactive packages must prove a visible state-changing control before trusted completion. |
...
| Runtime posture | Dockerized PostgreSQL/pgvector and NATS remain the reusable data-plane norm with Core and Interface run locally from source for proof. Ollama and local media generators remain independently managed providers. Full Compose app containers, Kubernetes, and WSL remain explicit release-proof lanes. |
| Delivery target | Trusted Outcome Journey and exact release-candidate certification are complete, and local `dev -> main` promotion is complete. Current development is tightening complex generated-output delivery so Soma can create a profile-backed temporary team, hand work to the bus, return a retained output, require generic app-package interaction proof, and truthfully expose validation failure as repair-needed work when output is not usable. |
| Main release risk | Simple deterministic package generation and live opening pass, but complex generated app/game delivery still exposes quality and closure gaps: a live voxel-game team wrote files, later degraded on the approved result contract, and produced a blank canvas package. Soma must close the loop faster with progress, degradation, direct launch, and repair guidance. |
...
| Profile-backed complex app delivery | `ACTIVE` | Headed live proof created, saved, and activated a workspace Worker Profile; then Soma proposed and approved a temporary voxel game team through chat. The team now restores and subscribes to `swarm.team.{team_id}.internal.command` even when profile inputs include human-readable fields like `game_brief`. The live team moved from prior `queued` failure to active bus work and wrote isolated files under `E:\random\test\groups\qa-voxel-game-...`; after bounded correction it degraded with `result_contract_unsatisfied` because support files/readback/validation markers were missing. Direct browser probe showed the generated `index.html` was blank (`0` nonblack sampled pixels before and after ArrowRight), so the generated output is not acceptable. Runtime response shaping now suppresses completion-style text for unsatisfied result contracts and returns repair-needed wording while retaining partial artifacts for recovery. Interactive package preflight now requires an inspectable handler, visible primary-control language, a DOM/render/state-changing effect, and no unstarted self-scheduling render loop. Work stuck in `reviewing` receives a short validation recovery deadline and degrades as `runtime_validation_deadline_exceeded` rather than remaining ambiguous. | Rerun the full profile -> team -> retained app -> open/play proof until it returns either a verified playable package or a clear recovery card inside the user-facing review window, then tighten the visible progress copy only where live UX still feels noisy. |
...
1. Run the generic complex-app live GUI proof from Soma ask through approval, async team work, retained `project_package`, UI open, interaction state change, proof/revisit, and cleanup.
2. If the candidate fails, require one concise recovery card that says what failed, what remains trusted, and whether Soma should repair, retry validation, or archive.
3. When Forge is reachable with API mode enabled, run the explicit natural Soma media ask through generated image preview, saved group media path, proof, and revisit.
