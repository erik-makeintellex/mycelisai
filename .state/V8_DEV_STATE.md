...
| Active slice | P0.7b-D2b exact runtime-team manifest persistence is `IN_REVIEW` on its feature branch. New teams persist their complete effective manifest before acknowledgement, restore exact Worker Profile lineage after Core restart, reject conflicting same-id replacement, and leave later activation/rollback prospective. Durable single-team and mission cleanup use the same storage boundary. The remaining P0.7b gate is live Soma create/activate/use/revisit acceptance. P0.9 remains `IN_REVIEW` only for manual novice acceptance. |
...
2. Integrate P0.7b-D2b exact runtime-team restoration, then run the live Soma Worker Profile create/activate/use/revisit journey with activation B and rollback A affecting only later teams.
