...
| Active slice | P0.3a bounded discovery and Outcome Templates is `COMPLETE`. Natural configured asks now cover preview, governed save, activation, reload, exact template application, and scoped cleanup in one authenticated visible Soma journey. P0.9 deterministic package delivery is the active implementation target. |
...
1. Make package-contract correction deterministic using the accepted Outcome Template brief so workers finish required writes, entrypoint readback, and validation within the bounded loop.
2. Rerun the full headed Ask -> Clarify -> Approve -> Execute -> Steer -> Deliver -> Open -> Revisit journey and complete manual novice acceptance.
3. Extend the proven configuration path through P0.7b adapters, beginning with the smallest reusable profile/action family rather than widening every family at once.
4. Review external-mutation recovery and execute P0.12 certification only if every release gate passes.
