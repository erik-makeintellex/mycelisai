...
| Field | State |
| --- | --- |
| Current committed checkpoint | `Correlate async team result signals` |
| Working tree at checkpoint | Active async team output slice: Team Ask queues non-blocking work, correlated status/result signals preserve `work_item_id`, and result payloads with `outputs[]` or normalized `output_refs[]` now project retained workspace paths/URLs/proof refs onto the original Active Work row. Inline artifact-ledger insertion is not part of this slice; teams should emit real retained paths or URLs when an output is meant to be openable. |
...
| Main release risk | Async team ask acceptance, correlated status/result projection, and retained output-ref projection are now operator-visible, but full release-candidate proof still needs richer team-specific output views and an explicit artifact-ledger policy for inline team-generated artifacts |
...
| UI Workflow/Expression | `IN_REVIEW` | Schedule Rules and Approvals are complete. Dashboard now exposes a `Work contexts` strip that switches root Soma and team-focused lanes while chat, active work, retained outputs, and proof move together. Ask Team now appears as queued non-blocking work immediately, Active Work polls active durable rows until terminal state, correlated team signals can advance queued rows to output-ready/degraded, and retained `outputs[]`/`output_refs[]` from result signals are projected back onto the original row for openable evidence chips. Live login/dashboard proof is green after aligning the homepage spec to the current `Soma environment` label. Dashboard workbench proof shows contained scrolling and retained-output/folder affordances on native services. Governance, package reload, media output, focused ComfyUI journey, and recovery/degradation are proven across focused specs. | Continue into richer team-specific output views and artifact-ledger policy for inline team outputs. | Focused Vitest, TypeScript, max-lines, and headed/browser proof where the workflow is visible. |
